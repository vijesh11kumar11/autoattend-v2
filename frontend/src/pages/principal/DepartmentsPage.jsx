...
export default function DepartmentsPage() {
  ...
  if (loading) return <div className="p-8 text-slate-400 text-sm animate-pulse">Loading departments…</div>;
  if (error)   return <div className="p-8 text-red-500 text-sm">{error}</div>;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">{departments.length} department{departments.length !== 1 ? 's' : ''} in college</p>
      </div>
        ...
        {departments.map((dept) => (
          <div key={dept.id} className="card p-5 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              ...
              <div className={`rounded-lg p-2 ${dept.defaulter_count > 0 ? 'bg-red-50' : 'bg-emerald-50'}`}>
                <p className={`font-bold text-lg ${dept.defaulter_count > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
