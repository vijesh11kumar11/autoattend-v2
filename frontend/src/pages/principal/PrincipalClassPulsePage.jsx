...
export default function PrincipalClassPulsePage() {
    ...
    api
      ...
      .then((r) => {
        ...
        return Promise.all(
          deptList.map((d) =>
            api
              .get(`/api/classpulse/hod/department-analytics?department_id=${d.id}`)
  ...
  const agg = useMemo(() => {
    ...
    const instEngagement = counted.length
      ? Math.round((counted.reduce((a, r) => a + r.avg_engagement, 0) / counted.length) * 10) / 10
      : 0;
    const instComprehension = counted.length
      ? Math.round((counted.reduce((a, r) => a + r.avg_comprehension, 0) / counted.length) * 10) /
        10
      : 0;
    ...
    const struggling =
      [...rows]
        .filter((r) => r.total_capsules > 0)
        .sort((a, b) => a.avg_comprehension - b.avg_comprehension)[0] || null;
    return {
  ...
  if (loading)
    return (
      <div className="p-8 text-slate-400 text-sm animate-pulse">Loading institution analytics…</div>
    );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Aggregated across {agg.institution.deptCount} departments
          </p>
        </div>
        <button
          onClick={exportCSV}
          className="text-xs font-semibold bg-slate-800 hover:bg-slate-900 text-white rounded-lg px-3 py-2"
        >
          ⬇️ Export CSV
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-violet-100 shadow-sm p-4 grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Tile
          ...
          value={`${agg.institution.instEngagement}%`}
          tone={pctText(agg.institution.instEngagement)}
        />
        <Tile
          icon="🧠"
          label="Avg Comprehension"
          value={`${agg.institution.instComprehension}%`}
          tone={pctText(agg.institution.instComprehension)}
        />
        <Tile
          icon="⚠️"
          label="Students at Risk"
          value={agg.institution.totalAtRisk}
          tone={agg.institution.totalAtRisk > 0 ? 'text-red-600' : 'text-slate-700'}
        />
      </div>
        ...
        {agg.topSubject ? (
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
            ...
            <p className="text-xs text-emerald-700">
              {agg.topSubject.dept_name} · {agg.topSubject.teacher_name || '—'}
            </p>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
        <h3 className="text-sm font-bold text-slate-800 mb-3">
          📊 Department Comparison (worst comprehension first)
        </h3>
        {agg.rows.length === 0 ? (
              ...
              <tbody>
                {agg.rows.map((r) => (
                  <tr key={r.dept_id} className="border-b border-slate-50">
                    ...
                    <td className={`py-2 pr-2 font-bold ${pctText(r.avg_comprehension)}`}>
                      {r.avg_comprehension}%
                    </td>
                    <td
                      className={`py-2 font-bold ${r.at_risk > 0 ? 'text-red-600' : 'text-slate-700'}`}
                    >
                      {r.at_risk}
                    </td>
                  </tr>
