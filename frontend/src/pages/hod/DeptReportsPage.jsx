...
function MonthlyExcelCard({ subjects }) {
  ...
  async function download() {
    ...
    try {
      ...
      downloadBlob(
        res.data,
        `attendance_${subj?.code || subjectId}_${year}_${String(month).padStart(2, '0')}.xlsx`
      );
    } catch {
      setError('Download failed.');
    } finally {
...
function DefaultersTable({ subjects }) {
  ...
  const load = useCallback(() => {
    ...
    api
      .get('/reports/hod/defaulters', { params })
...
export default function DeptReportsPage() {
  ...
  useEffect(() => {
    Promise.all([api.get('/reports/hod/students'), api.get('/reports/hod/subjects')])
      .then(([s, sub]) => {
        setStudents(s.data);
