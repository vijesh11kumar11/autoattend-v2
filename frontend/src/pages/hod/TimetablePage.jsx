...
const DAY_COLORS = {
  monday: 'bg-blue-50 border-blue-200', tuesday: 'bg-emerald-50 border-emerald-200',
  wednesday: 'bg-purple-50 border-purple-200', thursday: 'bg-amber-50 border-amber-200',
  friday: 'bg-red-50 border-red-200', saturday: 'bg-slate-50 border-slate-200',
};
...
export default function TimetablePage() {
  ...
  const showFlash = (msg, type = 'ok') => { setFlash(msg); setFlashType(type); };
  ...
  useEffect(() => {
    ...
    api.get('/hod/dashboard').then(r => setSubjects(r.data?.subjects || [])).catch(() => {});
  }, []);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      load();
    } catch (err) { showFlash(err.response?.data?.detail || 'Upload failed.', 'err'); }
  ...
  const handleExport = async () => {
    ...
    } catch { showFlash('Export failed.', 'err'); }
  ...
  Object.values(entries).flat().forEach(e => allTimes.add(e.start_time));
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <label className={`px-4 py-1.5 text-sm rounded-lg cursor-pointer
                    ...
                    <td key={day} className="p-1 border align-top min-w-[120px]">
                      {dayEntries.length > 0 ? dayEntries.map(e => (
                        <div key={e.timetable_id}
                             ...
                             style={e.color_tag ? { borderLeftColor: e.color_tag, borderLeftWidth: 4 } : {}}>
                          <p className="font-semibold text-slate-700 truncate">{e.subject_name}</p>
                          <p className="text-slate-400">{e.subject_code}</p>
                          <p className="text-slate-500 mt-1">👨‍🏫 {e.teacher_name}</p>
                          {e.section_name && <p className="text-slate-400">👥 {e.section_name}</p>}
                          <div className="flex justify-between mt-1">
                            <span className="font-mono">{e.start_time}–{e.end_time}</span>
                            <span>🏫 {e.room || '—'}</span>
                          </div>
                          {e.is_lab && <span className="inline-block mt-1 px-1 py-0.5 bg-purple-100 text-purple-600 rounded text-[10px]">LAB</span>}
                        </div>
                      )) : (
                        <button
      ...
      {modal && (
        <EntryModal
          ...
          onDeleted={(id) => { handleDelete(id); setModal(null); }}
          showFlash={showFlash}
        />
      )}
    </div>
...
function EntryModal({ modal, teachers, subjects, sections, onClose, onSaved, onDeleted, showFlash }) {
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="px-5 py-4 border-b flex items-center justify-between">
          <h3 className="font-semibold text-slate-700">{isEdit ? '✏️ Edit Period' : '➕ Add Period'}</h3>
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="w-full border rounded-lg px-3 py-2 text-sm" value={form.subject_id}
            ...
            <div>
              ...
              <select className="w-full border rounded-lg px-3 py-2 text-sm" value={form.teacher_id}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <select className="w-full border rounded-lg px-3 py-2 text-sm" value={form.day_of_week}
            ...
            <div>
              ...
              <input type="time" className="w-full border rounded-lg px-3 py-2 text-sm" value={form.start_time}
                     onChange={e => set('start_time', e.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">End Time</label>
              <input type="time" className="w-full border rounded-lg px-3 py-2 text-sm" value={form.end_time}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <select className="w-full border rounded-lg px-3 py-2 text-sm" value={form.section_id}
            ...
            <div>
              ...
              <input className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Room 101"
            ...
            <div>
              ...
              <input type="number" min={1} max={12} className="w-full border rounded-lg px-3 py-2 text-sm"
                     value={form.period_number} onChange={e => set('period_number', e.target.value)} />
            </div>
          </div>
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 text-sm text-slate-600">
              <input type="checkbox" checked={form.is_lab} onChange={e => set('is_lab', e.target.checked)} className="rounded" />
              Lab session
            </label>
            <div className="flex items-center gap-2">
          ...
          <div className="flex justify-between pt-2">
            <div>
              {isEdit && (
                <button onClick={() => onDeleted(e.timetable_id)}
                        className="px-4 py-2 text-sm text-red-500 hover:text-red-700">🗑️ Delete</button>
              )}
            </div>
