...
function FaceReenrollPage() {
  ...
  const load = () => {
    ...
    setError('');
    api
      .get('/api/face/admin/students', { params: { only_enrolled: onlyEnrolled } })
    ...
    <div className="space-y-4">
      {flash && (
        <div className="card px-4 py-2.5 bg-amber-50 text-amber-800 text-sm flex items-center justify-between">
          <span>{flash}</span>
          <button onClick={() => setFlash('')} className="opacity-60 hover:opacity-100">
            ✕
          </button>
        </div>
      )}
      ...
      <div className="card p-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          <p className="text-xs text-slate-500">
            Clear a student's face data so they can re-enroll. All resets are audited
            (FaceChangeLog).
          </p>
        </div>
      ...
      {target && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-5 space-y-4">
            <h3 className="text-base font-bold text-slate-800">Reset face for {target.name}?</h3>
            <p className="text-xs text-slate-500">
              This clears the Azure face record. {target.name} will be prompted to enroll their face
              again on next login. The action is irreversible and is logged.
            </p>
            <textarea
              rows={3}
              value={reason}
    ...
    <div className="card p-4 border-violet-100">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {stats.total_capsules || 0} capsules across department
            </p>
          </div>
        </div>
        <a
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
        <Metric
          ...
          tone={
            (stats.avg_engagement_pct ?? 0) >= 70
              ? 'text-emerald-600'
              : (stats.avg_engagement_pct ?? 0) >= 50
                ? 'text-amber-600'
                : 'text-red-600'
          }
        />
        <Metric
          ...
          tone={
            (stats.avg_comprehension_pct ?? 0) >= 70
              ? 'text-emerald-600'
              : (stats.avg_comprehension_pct ?? 0) >= 50
                ? 'text-amber-600'
                : 'text-red-600'
          }
        />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-1">
              {data.section_overview.slice(0, 3).map((s) => (
                <div key={s.section_id} className="flex items-center justify-between text-xs">
                  ...
                  <span
                    className={`font-bold ${s.avg_pct >= 75 ? 'text-emerald-600' : s.avg_pct >= 60 ? 'text-amber-500' : 'text-red-500'}`}
                  >
                    {s.avg_pct}%
                  </span>
                </div>
              ))}
              {data.section_overview.length > 3 && (
                <p className="text-[10px] text-slate-400">
                  +{data.section_overview.length - 3} more…
                </p>
              )}
            </div>
          ) : (
        ...
        >
          ...
          {data.tutor_stats?.unassigned_students > 0 && (
            <p className="text-xs text-amber-600 mt-1 font-medium">
              ⚠️ {data.tutor_stats.unassigned_students} unassigned
            </p>
          )}
        </a>
        ...
        <a
          href="/hod/disputes"
          className={`card p-4 hover:shadow-md transition cursor-pointer ${data.pending_disputes_count > 0 ? 'border-amber-200 bg-amber-50/30' : ''}`}
        >
          <div className="flex items-center gap-2 mb-2">
          ...
          <p
            className={`text-xl font-bold ${data.pending_disputes_count > 0 ? 'text-amber-600' : 'text-slate-400'}`}
          >
            {data.pending_disputes_count}
          </p>
                ...
                return (
                  <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {sess ? (
                        <span
                          className={`badge ${sess.status === 'active' ? 'badge-success' : 'badge-secondary'}`}
                        >
                          {sess.status}
                        </span>
                      ) : (
              ...
              {subjects.map((s) => (
                <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-600">
                    {s.teacher_name || <span className="text-slate-300">Unassigned</span>}
                  </td>
  ...
  return (
    <div id="pending-approvals" className="card overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
        ...
        {actionMsg && <span className="text-xs text-emerald-600 font-medium">{actionMsg}</span>}
      </div>
