  ...
  return (
    <div className="space-y-5">
      ...
      {msg && (
        ...
        >
          <span>{msg}</span>
          <button className="opacity-50 hover:opacity-100" onClick={() => setMsg('')}>
            ✕
          </button>
        </div>
      )}
  ...
  const handleExport = async () => {
    ...
    try {
      const r = await api.post(
        `/tutor/export-assignments-excel?academic_year=${encodeURIComponent(year)}`,
        null,
        {
          responseType: 'blob',
        }
...
function AssignmentsTab({ year, flash }) {
  ...
  const load = () => {
    setLoading(true);
    api
      .get('/tutor/assignments', { params: { academic_year: year } })
    ...
    <div className="card overflow-hidden">
      <div className="px-5 py-3 bg-slate-50 border-b flex items-center justify-between flex-wrap gap-3">
        ...
        <input
          ...
          placeholder="Search tutor, student, roll…"
          value={search}
...
function AssignTab({ year, flash }) {
  ...
  useEffect(() => {
    ...
    api
      .get('/tutor/unassigned-students', { params: { academic_year: year } })
  ...
  const submitRollRange = async () => {
    ...
    try {
      ...
      const d = r.data;
      flash(
        `Assigned ${d.assigned} student(s).` +
          (d.conflicts?.length ? ` ${d.conflicts.length} conflict(s).` : '')
      );
    } catch (err) {
      flash(err.response?.data?.detail || 'Failed.', 'err');
    } finally {
  ...
  const submitSection = async () => {
    ...
    try {
      ...
      const d = r.data;
      flash(
        `Assigned ${d.assigned} to ${d.tutor_name} in section ${d.section_name}. ${d.skipped} skipped.`
      );
    } catch (err) {
      flash(err.response?.data?.detail || 'Failed.', 'err');
    } finally {
  ...
  const submitExcel = async () => {
    ...
    try {
      ...
      const d = r.data;
      flash(
        `Excel: ${d.success_count} assigned, ${d.skipped_count} skipped, ${d.failed_rows?.length || 0} failed.`
      );
      setExcelFile(null);
    } catch (err) {
      flash(err.response?.data?.detail || 'Upload failed.', 'err');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-5 space-y-4">
        ...
        {mode !== 'excel' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                className="w-full border rounded-lg px-3 py-2 text-sm"
                placeholder="e.g. Section A rows 1–30"
                value={note}
        ...
        {mode === 'manual' && (
          <>
            <div className="border rounded-lg max-h-72 overflow-y-auto p-3 space-y-1">
              {students.length > 0 ? (
                students.map((s) => (
                  <label
                    key={s.id}
                    className={`flex items-center gap-3 p-1.5 rounded cursor-pointer
            ...
            <button
              ...
              disabled={loading || !tutorId || selectedIds.size === 0}
        ...
        {mode === 'roll' && (
          <div className="space-y-4">
            ...
            <button
              onClick={submitRollRange}
              disabled={loading || !tutorId || !rollStart || !rollEnd}
              className="px-5 py-2 bg-[#1a237e] text-white text-sm rounded-lg hover:bg-[#283593] disabled:opacity-50"
            >
        ...
        {mode === 'section' && (
          <div className="space-y-4">
                ...
                >
                  ...
                  {sections.map((s) => (
                    <option key={s.id} value={s.id}>
                      {s.name} — {s.course_name} Sem {s.semester}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <button
              ...
              disabled={loading || !tutorId || !sectionId}
              className="px-5 py-2 bg-[#1a237e] text-white text-sm rounded-lg hover:bg-[#283593] disabled:opacity-50"
            >
        ...
        {mode === 'excel' && (
          <div className="space-y-4">
            <p className="text-sm text-slate-500">
              ...
              <code className="bg-slate-100 px-1 rounded">roll_number</code>,{' '}
    ...
    <div className="card overflow-hidden">
      <div className="px-5 py-3 bg-amber-50 border-b flex items-center justify-between flex-wrap gap-3">
        <span className="text-sm text-amber-700 font-medium">
          ⚠️ {data.length} student(s) without a tutor for {year}
        </span>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={assignNow}
            disabled={assigning || !tutorId || selectedIds.size === 0}
      ...
      {data.length > 0 ? (
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {data.map((s) => (
                <tr
                  key={s.id}
                  className={`hover:bg-slate-50 ${selectedIds.has(s.id) ? 'bg-blue-50' : ''}`}
                >
        ...
        <div className="p-10 text-center text-emerald-500 text-sm">
          ✅ All students have a tutor assigned for {year}.
        </div>
      )}
    </div>
  );
