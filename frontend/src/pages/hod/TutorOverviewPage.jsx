            ...
            <div key={t.tutor_id} className="card p-5 space-y-3">
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shrink-0">
                  {(t.tutor_name || 'T').split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase()}
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className={`rounded-lg p-2 text-center ${t.defaulter_count > 0 ? 'bg-red-50' : 'bg-emerald-50'}`}>
                  <p className={`text-lg font-bold ${t.defaulter_count > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                    {t.defaulter_count}
                  </p>
                  <p className="text-[10px] text-slate-500 uppercase">Defaulters</p>
                </div>
                <div className={`rounded-lg p-2 text-center ${t.pending_leaves > 0 ? 'bg-amber-50' : 'bg-slate-50'}`}>
                  <p className={`text-lg font-bold ${t.pending_leaves > 0 ? 'text-amber-600' : 'text-slate-400'}`}>
                    {t.pending_leaves}
                  </p>
              ...
              {t.defaulter_count > 0 && (
                <p className="text-xs text-red-600 bg-red-50 px-3 py-1.5 rounded-lg font-medium">
                  🚨 {t.defaulter_count} ward{t.defaulter_count > 1 ? 's' : ''} below {75}% threshold
                </p>
              )}
            </div>
          ))}
