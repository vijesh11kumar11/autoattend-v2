...
function StatusBadge({ status }) {
  switch (status) {
    ...
    default:        return <span className="badge badge-secondary capitalize">{status}</span>;
  }
}
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full space-y-4">
        ...
        <p className="text-sm text-slate-600">
          ...
          <span className="font-bold text-red-600">{count} student{count !== 1 ? 's' : ''}</span>{' '}
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button onClick={onConfirm} className="btn-primary  flex-1 text-sm bg-red-600 hover:bg-red-700">
            Send {count} Alerts
          </button>
        </div>
      </div>
      ...
      {results.filter(r => r.status === 'failed').length > 0 && (
        <div className="max-h-32 overflow-y-auto text-xs text-red-500 space-y-0.5">
          {results.filter(r => r.status === 'failed').map((r, i) => (
            <p key={i}>{r.student_name || `Student #${r.student_id}`}: {r.reason || 'Unknown error'}</p>
          ))}
        </div>
      )}
    </div>
...
function BulkAlertSection() {
  ...
  useEffect(() => {
    api.get('/alerts/hod/defaulters/count')
  ...
  return (
    <div className="card p-5 space-y-4">
      ...
      <button
        ...
        onClick={() => { setResult(null); setShowDlg(true); }}
  ...
  async function handleSend(e) {
    ...
    try {
      const { data } = await api.post('/alerts/hod/send-custom',
        { student_id: studentId, message: message.trim() });
      setSuccess(data.status === 'sent'
        ? `✓ Message sent to ${data.student_name}'s parent.`
        : `Alert logged but delivery failed: ${data.reason}`);
      setMessage(''); setStudentId(''); setQuery('');
    } catch (err) {
  ...
  return (
    <div className="card p-5 space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">Send a personalised WhatsApp to one student's parent.</p>
      </div>
      ...
      <form onSubmit={handleSend} className="space-y-3">
        ...
        <div className="relative">
          ...
          />
          {filtered.length > 0 && (
            <div className="absolute z-10 mt-1 w-full bg-white border border-slate-200 rounded-xl
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-slate-100 flex flex-wrap items-center gap-3">
        ...
        <select value={statFilt} onChange={e => setStatFilt(e.target.value)}
            ...
            <tbody className="divide-y divide-slate-50">
              {rows.map(r => (
                <tr key={r.id} className="hover:bg-slate-50">
                  <td className="px-4 py-2.5 text-xs text-slate-500 whitespace-nowrap">{formatDate(r.sent_at)}</td>
