  ...
  return (
    <svg width={size} height={size} className="-rotate-90">
      ...
      <text x="50%" y="50%" dominantBaseline="central" textAnchor="middle"
        className="fill-slate-700 text-sm font-bold" transform={`rotate(90 ${size / 2} ${size / 2})`}>
        {pct}%
      </text>
    </svg>
  );
    ...
    <div className="space-y-6">
      ...
      {sections.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {sections.map(sec => (
            <div key={sec.section_id} className={`card p-5 border ${sec.avg_pct < 65 ? 'border-red-200 bg-red-50/30' : ''}`}>
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold text-slate-800">Section {sec.section_name}</h3>
                  <p className="text-xs text-slate-400">Semester {sec.semester} · {sec.student_count} students</p>
                </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                ...
                <div className={`text-center rounded-lg p-2 ${sec.defaulter_count > 0 ? 'bg-red-50' : 'bg-emerald-50'}`}>
                  <p className={`text-xl font-bold ${sec.defaulter_count > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                    {sec.defaulter_count}
                  </p>
              ...
              {sec.worst_subject && (
                <div className="mt-3 px-3 py-2 bg-amber-50 rounded-lg text-xs">
                  ...
                  <span className="text-amber-600">{sec.worst_subject.subject_name} — {sec.worst_subject.avg_pct}%</span>
                </div>
              )}
            </div>
          ))}
