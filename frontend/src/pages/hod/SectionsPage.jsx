...
export default function SectionsPage() {
  ...
  const loadCourses = () => {
    api.get('/reports/hod/subjects')
      .then(r => {
        const subjs = r.data || [];
        const map = {};
        subjs.forEach(s => { if (s.course_id && s.course_name) map[s.course_id] = s.course_name; });
  ...
  useEffect(() => { loadSections(); }, [courseFilter, semFilter]);

  // ── create / edit ────────────────────────────────────────────────
  const openCreate = () => {
    setEditing(null);
    setForm({ course_id: courseFilter !== 'all' ? courseFilter : '', semester: semFilter !== 'all' ? semFilter : '', name: '', max_strength: '' });
    setFormErr('');
    setShowModal(true);
  };
  ...
  const openEdit = (sec) => {
    setEditing(sec);
    setForm({ course_id: sec.course_id, semester: sec.semester, name: sec.name, max_strength: sec.max_strength || '' });
    setFormErr('');
    setShowModal(true);
  };
  ...
  const handleDelete = async (sec) => {
    if (!window.confirm(`Delete section "${sec.name}" (${sec.course_name} Sem ${sec.semester})? Students will be unassigned.`)) return;
    try {
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-3 bg-slate-50 border-b flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <select className="text-sm border rounded-lg px-3 py-1.5 text-slate-600"
        ...
        <div className="card overflow-hidden">
          <div className="px-5 py-3 bg-blue-50 border-b flex items-center justify-between">
            <span className="font-semibold text-blue-800">
              Students in Section {viewSection.name} — {viewSection.course_name} Sem {viewSection.semester}
            </span>
            <div className="flex items-center gap-2">
              {/* excel upload */}
              <input type="file" accept=".xlsx,.xls" id="excel-upload"
                     className="hidden" onChange={e => setExcelFile(e.target.files[0])} />
              <label htmlFor="excel-upload"
                     className="text-xs px-3 py-1.5 rounded bg-purple-50 text-purple-600 hover:bg-purple-100 cursor-pointer">
                📄 Upload Excel
              </label>
              {excelFile && (
                <button onClick={() => submitExcel(viewSection)} disabled={excelLoading}
              ...
              <button onClick={() => setViewSection(null)}
          ...
          {studentsLoading ? (
            <div className="p-6 text-center text-slate-400 text-sm animate-pulse">Loading students…</div>
                ...
                <tbody className="divide-y">
                  {sectionStudents.map(s => (
                    <tr key={s.id} className="hover:bg-slate-50">
                      <td className="px-5 py-2 text-sm font-mono text-slate-600">{s.roll_number || '—'}</td>
            ...
            <div className="p-6 text-center text-slate-400 text-sm">No students in this section yet.</div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            ...
            <form onSubmit={submitForm} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">Section Name</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">Max Strength (optional)</label>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)}
                        className="px-4 py-2 text-sm text-slate-500 hover:text-slate-700">Cancel</button>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col">
            <div className="px-5 py-4 border-b flex items-center justify-between">
              ...
              <button onClick={() => setShowAssign(false)} className="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            <div className="flex-1 overflow-y-auto p-5">
              {assignLoading ? (
                <div className="text-center text-slate-400 text-sm animate-pulse">Loading students…</div>
              ) : unassigned.length > 0 ? (
                ...
                <div className="text-center text-slate-400 text-sm">No unassigned students available.</div>
              )}
            </div>
            <div className="px-5 py-3 border-t flex justify-end gap-3">
              <button onClick={() => setShowAssign(false)}
                      className="px-4 py-2 text-sm text-slate-500 hover:text-slate-700">Cancel</button>
              <button onClick={submitAssign} disabled={selectedIds.size === 0 || assignLoading}
