...
export default function TeacherPreClassBrief() {
  ...
  useEffect(() => {
    api.get('/live/pre-class-brief', { params: { session_id: sessionId } })
  ...
  const sendResource = async (studentId) => {
    try { await api.post('/notifications/push', { user_id: studentId, message: 'Pre-class resource sent.' }); }
  ...
  return (
    <div className="space-y-5 max-w-3xl mx-auto">
      <div className={`bg-gradient-to-r ${VIOLET} rounded-2xl p-6 text-white shadow-lg`}>
        ...
        <p className="text-white/80 mt-1 text-sm">{brief?.subject_name || ''} {brief?.scheduled_at ? `· ${new Date(brief.scheduled_at).toLocaleString()}` : ''}</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-violet-100 p-5 shadow-sm">
        ...
        <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-violet-500 to-fuchsia-500 transition-all duration-700"
          ...
          {needs.map(s => (
            <li key={s.student_id || s.id} className="py-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">{s.reason || s.weakness || 'Needs follow-up'}</p>
              </div>
              <button onClick={()=>sendResource(s.student_id||s.id)}
      ...
      <div className="bg-white rounded-2xl border border-violet-100 p-5 shadow-sm">
        ...
        <ul className="space-y-2">
          {concepts.length === 0 && <li className="text-sm text-slate-400">AI hasn't flagged any concepts yet.</li>}
          {concepts.map((c,i) => (
