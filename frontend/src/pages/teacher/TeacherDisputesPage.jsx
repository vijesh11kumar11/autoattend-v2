...
export default function TeacherDisputesPage() {
  ...
  const loadDisputes = () => {
    setLoading(true);
    api.get('/teacher/disputes/pending')
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 border-b">
                <tr>
                  {['Student', 'Subject', 'Session Date', 'Reason', 'Proof/Note', 'Status', 'Action'].map(h => (
              ...
              <tbody className="divide-y">
                {disputes.map(d => (
                  <tr key={d.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-xs text-slate-400 italic max-w-[150px] truncate">{d.proof_note || '—'}</td>
                    <td className="px-4 py-3">
          ...
          <div className="card w-full max-w-md p-6 space-y-4 m-4">
            ...
            <div>
              ...
              <textarea value={note} onChange={e => setNote(e.target.value)}
                placeholder="Add a resolution note…" rows={3} maxLength={500}
            ...
            <button onClick={handleResolve} disabled={submitting}
              ...
              {submitting ? '⏳ Processing…' : action === 'approve' ? '✅ Approve & Correct Attendance' : '❌ Reject Dispute'}
            </button>
          </div>
        </div>
      )}
