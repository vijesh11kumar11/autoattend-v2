            ...
            {loading ? (
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                  ...
                  <div><span className="text-slate-500">Absent:</span> <strong className="text-red-500">{details.absent_count}</strong></div>
                  <div><span className="text-slate-500">Status:</span> <strong>{details.status}</strong></div>
                </div>

                {flash && <p className="text-xs font-medium text-blue-600 bg-blue-50 rounded px-3 py-1">{flash}</p>}
                      ...
                      <tbody className="divide-y">
                        {details.students.map(st => (
                          <tr key={st.student_id} className="hover:bg-white">
                            <td className="px-3 py-2 font-mono text-xs text-slate-500">{st.roll_number || '—'}</td>
                            <td className="px-3 py-2 font-medium text-slate-700">{st.name}</td>
                            <td className="px-3 py-2">
                              <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
                                st.status === 'present' ? 'bg-emerald-100 text-emerald-700'
                                : st.status === 'late' ? 'bg-amber-100 text-amber-700'
                                : 'bg-red-100 text-red-700'
                              }`}>{st.status}</span>
                            </td>
                            <td className="px-3 py-2 text-xs text-slate-400">
                              {st.face_verified && '👤'} {st.gps_verified && '📍'} {st.bluetooth_verified && '📶'}
                              {!st.face_verified && !st.gps_verified && !st.bluetooth_verified && '—'}
                            </td>
                            <td className="px-3 py-2">
                              {overriding === st.student_id ? (
                                <div className="flex flex-col gap-1">
                                  <input
                                    type="text" placeholder="Reason (min 5 chars)"
                                  ...
                                  <div className="flex gap-1">
                                    {st.status !== 'present' && (
                                      <button onClick={() => handleOverride(st.student_id, 'present')}
                                        className="text-[10px] px-2 py-0.5 bg-emerald-600 text-white rounded hover:bg-emerald-700">
                                        ✓ Present
                                      </button>
                                    )}
                                    {st.status !== 'absent' && (
                                      <button onClick={() => handleOverride(st.student_id, 'absent')}
                                ...
                                <button onClick={() => setOverriding(st.student_id)}
