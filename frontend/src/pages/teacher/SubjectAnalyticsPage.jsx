  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <button onClick={() => navigate('/teacher/dashboard')} className="text-sm text-slate-400 hover:text-slate-600 mb-1">
            ← Back to Dashboard
          </button>
          <h1 className="text-xl font-bold text-slate-800">{subj.name}</h1>
          <p className="text-sm text-slate-400 font-mono">{subj.code} · Semester {subj.semester}</p>
        </div>
        {/* Health Score Badge */}
        {health?.health_score != null && (
          <div className={`text-center px-5 py-3 rounded-xl ${
            health.health_score >= 75 ? 'bg-green-50 text-green-700' :
            health.health_score >= 50 ? 'bg-amber-50 text-amber-700' :
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div className="card p-4 text-center">
          <p className="text-2xl font-bold text-slate-800">{analytics?.avg_session_duration ?? '—'}</p>
      ...
      {health?.breakdown && (
        <div className="card p-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { label: 'Avg Attendance', value: health.breakdown.avg_attendance, max: 100, weight: '40%' },
              { label: 'Consistency', value: health.breakdown.consistency_score, max: 100, weight: '20%' },
              { label: 'Trend', value: health.breakdown.trend_score, max: 100, weight: '20%' },
              { label: 'Low Defaulters', value: health.breakdown.defaulter_score, max: 100, weight: '20%' },
            ].map(m => (
              <div key={m.label} className="text-center">
                <div className="relative w-14 h-14 mx-auto mb-1">
                  <svg className="w-14 h-14 -rotate-90" viewBox="0 0 36 36">
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke="#e2e8f0" strokeWidth="3" />
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke={m.value >= 70 ? '#22c55e' : m.value >= 40 ? '#f59e0b' : '#ef4444'}
                      strokeWidth="3" strokeDasharray={`${m.value} 100`} strokeLinecap="round" />
                  </svg>
                  <span className="absolute inset-0 flex items-center justify-center text-xs font-bold">{Math.round(m.value)}</span>
                </div>
                <p className="text-[10px] text-slate-500">{m.label} ({m.weight})</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {dayPattern.length > 0 && (
        <div className="card p-4">
          <p className="text-sm font-semibold text-slate-700 mb-3">📊 Day-of-Week Attendance Pattern</p>
        ...
        <div className="card overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {students.map((s, i) => (
                <tr key={s.student_id} className={`hover:bg-slate-50 ${s.needs_attention ? 'bg-red-50/30' : ''}`}>
                  ...
                  <td className="px-4 py-2 text-center">
                    <span className={`text-sm font-bold ${s.pct >= 75 ? 'text-green-600' : 'text-red-600'}`}>
                      {s.pct}%
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-2 text-center">
                    {s.needs_attention ? (
                      <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded-full text-[10px] font-semibold">At Risk</span>
                    ) : (
                      <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-[10px] font-semibold">OK</span>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {tab === 'anomalies' && (
        <div className="space-y-3">
          {anomalyList.length === 0 ? (
            <div className="card p-8 text-center text-slate-400 text-sm">No anomalies detected. All clear!</div>
          ) : anomalyList.map((a, i) => (
            <div key={i} className={`card p-4 border-l-4 ${
              ...
              {a.student && (
                <p className="text-xs text-slate-400 mt-1">Student: {a.student} {a.roll_number && `(${a.roll_number})`}</p>
              )}
            </div>
          ))}
        </div>
      ...
      {tab === 'followup' && (
        <div className="space-y-3">
          {followupList.length === 0 ? (
            <div className="card p-8 text-center text-slate-400 text-sm">No follow-up suggestions for this week.</div>
          ) : followupList.map((f, i) => (
            <div key={i} className={`card p-4 border-l-4 ${
              f.priority === 'high' ? 'border-red-500' : f.priority === 'medium' ? 'border-amber-400' : 'border-blue-300'
            }`}>
              <div className="flex items-center justify-between">
                <div>
              ...
              <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                <span>Absent {f.absent_count}/{f.total_sessions} sessions</span>
                {f.has_approved_leave && <span className="text-green-600">✓ Has approved leave</span>}
              </div>
              {f.absent_dates?.length > 0 && (
                <p className="text-[10px] text-slate-300 mt-1">Dates: {f.absent_dates.join(', ')}</p>
              )}
            </div>
          ))}
        </div>
