    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="p-4">
                ...
                <div className="mt-3 flex gap-2">
                  {currentClass.can_start_attendance && (
                    <button onClick={() => handleStart(currentClass.timetable_id)} disabled={starting}
            ...
            {todaySchedule.length > 0 ? (
              <div className="divide-y">
                {todaySchedule.map(slot => (
                  <div key={slot.timetable_id} className="px-4 py-2.5 flex items-center justify-between hover:bg-slate-50">
                    <div className="flex items-center gap-2">
                      {slot.color_tag && (
                        <div className="w-1.5 h-8 rounded-full" style={{ backgroundColor: slot.color_tag }} />
                      )}
                      <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${STATUS_BG[slot.session_status] || STATUS_BG.not_started}`}>
                  ...
                  return (
                    <div key={s.session_id} className="px-4 py-2 flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <span className="text-sm font-bold">{s.present}/{s.total}</span>
                        <span className={`ml-1 text-xs ${pct >= 75 ? 'text-green-600' : 'text-red-500'}`}>({pct}%)</span>
                      </div>
                    </div>
                  );
                })}
            ...
            <div key={s.subject_id}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-mono">{s.code} · Sem {s.semester}</p>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 mt-3 text-center">
                ...
                <div className={`rounded-lg py-2 ${s.avg_attendance_pct >= 75 ? 'bg-emerald-50' : 'bg-red-50'}`}>
                  <p className={`text-lg font-bold ${s.avg_attendance_pct >= 75 ? 'text-emerald-700' : 'text-red-700'}`}>
                    {s.avg_attendance_pct}%
                  </p>
                  <p className={`text-[10px] ${s.avg_attendance_pct >= 75 ? 'text-emerald-400' : 'text-red-400'}`}>Avg Att.</p>
                </div>
              </div>
              {s.section_name && (
              ...
              {s.last_session_date && (
                <p className="text-[10px] text-slate-300 mt-1">Last session: {s.last_session_date}</p>
              )}
            </div>
          )) : (
        ...
        <div className="space-y-4">
          {/* Ward Summary */}
          {ward && ward.is_tutor && (
            <div className="card p-4 cursor-pointer hover:shadow-md transition" onClick={() => navigate('/teacher/ward-students')}>
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400">
                    ...
                    {ward.needs_attention_count > 0 && (
                      <span className="text-red-500 ml-1">· {ward.needs_attention_count} need attention</span>
                    )}
                  </p>
                </div>
              </div>
              {ward.pending_leaves > 0 && (
                <div className="mt-2 px-3 py-1.5 bg-amber-50 rounded-lg text-sm text-amber-700">
                  ✋ {ward.pending_leaves} pending leave request{ward.pending_leaves !== 1 ? 's' : ''}
                </div>
              )}
            </div>
          )}
          ...
          <div className="card overflow-hidden">
            ...
            <div className="p-3 grid grid-cols-2 gap-2">
              ...
              <button onClick={() => navigate('/teacher/leave-requests')}
              ...
              <button onClick={() => navigate('/teacher/twm')}
