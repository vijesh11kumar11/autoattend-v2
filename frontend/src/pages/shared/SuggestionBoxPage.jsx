...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════ */
const ROLE_CFG = {
  student:   { icon:'💡', title:'Suggestion Box',               sub:'Your voice matters — share feedback anonymously',       bg:'from-indigo-600 to-purple-600' },
  teacher:   { icon:'📬', title:'Student Feedback',              sub:'See what your students are saying about your classes',   bg:'from-blue-600 to-cyan-600' },
  hod:       { icon:'📊', title:'Department Feedback Analysis',  sub:'AI-powered insights from your department',              bg:'from-[#1a237e] to-indigo-700' },
  principal: { icon:'🏛️', title:'Institution Feedback Dashboard', sub:'Complete picture of your institution\'s pulse',         bg:'from-[#4a0000] to-purple-900' },
};
...
const TAB_LABELS = { ai:'AI Analysis', all:'All Feedback', submit:'Submit Feedback', my:'My Submissions', 'teacher-fb':'Student Feedback' };
...
const PRIORITY_OPTS = [
  { key:'low',    icon:'💬', label:'Just sharing',           color:'bg-blue-50 border-blue-200 text-blue-700' },
  { key:'medium', icon:'⚡', label:'This needs attention',   color:'bg-orange-50 border-orange-200 text-orange-700' },
  { key:'high',   icon:'🔥', label:'This is urgent',         color:'bg-red-50 border-red-200 text-red-700' },
];
...
const LOADING_STEPS = ['Collecting all feedback...','Sending to Gemini AI...','Analysing patterns...','Generating insights...'];
    ...
    <div className="space-y-6">
      ...
      <div className={`bg-gradient-to-r ${cfg.bg} rounded-2xl p-6 md:p-8 text-white`}>
        <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-3">
          <span className="text-3xl">{cfg.icon}</span>{cfg.title}
        </h1>
...
function SubmitTab({ role }) {
  ...
  useEffect(() => {
    if (role === 'student') {
      api.get('/student-portal/subjects').then(r => {
        setSubjects(Array.isArray(r.data) ? r.data : r.data.subjects || []);
      }).catch(() => {});
    }
  }, [role]);

  const cats = useMemo(() => role === 'teacher' ? [...CATEGORIES, ...TEACHER_EXTRA_CATS] : CATEGORIES, [role]);
  ...
  const handleSubmit = async () => {
    ...
    if (message.trim().length < 20) { setError('Feedback must be at least 20 characters'); return; }
    setError('');
    setSubmitting(true);
    try {
      ...
      setSuccess(true);
      setCategory(''); setMessage(''); setPriority('low'); setSubjectId('');
    ...
    return (
      <div className="bg-white rounded-xl border border-emerald-200 shadow-sm p-10 text-center space-y-4">
        ...
        <p className="text-sm text-slate-500 max-w-md mx-auto">
          Your feedback has been submitted anonymously. Thank you for helping improve our institution!
        </p>
        <button onClick={() => setSuccess(false)} className="px-6 py-2 bg-indigo-600 text-white rounded-lg text-sm hover:bg-indigo-700">
          Submit Another
        </button>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-blue-800">Your identity is completely anonymous.</p>
          <p className="text-xs text-blue-600 mt-0.5">No one — not your teacher, HOD, or principal — can ever see who submitted this feedback.</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
          {cats.map(c => (
            <button key={c.key} onClick={() => { setCategory(c.key); setError(''); }}
              ...
              <span className={`text-sm font-medium ${category === c.key ? 'text-indigo-700' : 'text-slate-700'}`}>{c.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {role === 'student' && subjects.length > 0 && (
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-2">Who is this about? (optional)</label>
      ...
      {/* Priority */}
      <div>
        <label className="block text-sm font-semibold text-slate-700 mb-3">How urgent is this?</label>
...
function MySubmissionsTab() {
  ...
  useEffect(() => {
    api.get('/suggestions/my-submissions').then(r => setItems(r.data)).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  if (!items.length) {
    return (
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-10 text-center">
        ...
        <p className="text-slate-400 text-sm">You haven't submitted any feedback yet. Your anonymous voice can help improve the institution!</p>
      </div>
    );
  }
...
function TeacherFeedbackTab() {
  ...
  useEffect(() => {
    api.get('/suggestions/teacher-feedback').then(r => setData(r.data)).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  if (!data?.grouped_feedback?.length) {
    return (
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-10 text-center">
        <div className="text-5xl mb-3">📭</div>
        <p className="text-slate-400 text-sm">No student feedback received yet for your subjects.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs text-blue-700">
        ℹ️ These are anonymous submissions from your students. Student identities are completely protected.
      </div>
      {data.grouped_feedback.map((group, gi) => (
        <div key={gi}>
          <div className="flex items-center gap-3 mb-3">
            <h3 className="text-sm font-bold text-slate-700">📚 {group.subject?.name || 'Unknown Subject'}</h3>
            {group.feedback?.length > 0 && (() => {
              ...
              const avg = sentiments.length === 0 ? null : pos > neg ? 'positive' : neg > pos ? 'negative' : 'neutral';
              return avg ? <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${SENTIMENT_BADGE[avg]}`}>{avg}</span> : null;
            })()}
            <span className="text-[10px] text-slate-400">{group.feedback?.length || 0} feedback</span>
          </div>
          <div className="space-y-2.5 pl-4 border-l-2 border-blue-200">
            {group.feedback.map(f => <SuggestionCard key={f.id} s={f} compact />)}
          </div>
        </div>
      ))}
    </div>
...
function AIAnalysisTab({ role }) {
  ...
  const endpoint = scope === 'institution' ? '/suggestions/institution-analysis' : '/suggestions/department-analysis';

  useEffect(() => {
    api.get(endpoint).then(r => { if (r.data.report) setReport(r.data.report); }).catch(() => {}).finally(() => setLoading(false));
  }, [endpoint]);
  ...
  useEffect(() => {
    ...
    setGenStep(0);
    let i = 0;
    const iv = setInterval(() => { i++; if (i < 4) setGenStep(i); else clearInterval(iv); }, 2500);
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-10 text-center space-y-6">
        ...
        <div className="space-y-3 max-w-xs mx-auto">
          {LOADING_STEPS.map((step, i) => (
            <div key={i} className={`flex items-center gap-3 text-sm transition-all duration-500 ${i <= genStep ? 'opacity-100' : 'opacity-20'}`}>
  ...
  if (!report) {
    return (
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-10 text-center space-y-4">
        ...
        <p className="text-sm text-slate-400 max-w-md mx-auto">Click Generate AI Analysis to get instant AI-powered insights from all feedback submissions.</p>
        {error && <p className="text-red-500 text-xs">{error}</p>}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="text-xs text-slate-400">
          Last generated: {report.generated_at ? new Date(report.generated_at).toLocaleString() : '—'}
          <span className="ml-2 px-2 py-0.5 bg-slate-100 rounded-full">{report.ai_provider}</span>
        </div>
        <button onClick={generateReport} className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg text-xs font-semibold hover:opacity-90">
          🤖 Regenerate
        </button>
      </div>
      {error && <p className="text-red-500 text-xs">{error}</p>}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Urgent Items" value={(rd.urgent_attention || []).length} icon="🚨"
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
        ...
        {rd.trend_insight && (
          <p className="text-xs text-slate-500 mt-3 pt-3 border-t border-slate-100">📈 {rd.trend_insight}</p>
        )}
      </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {rd.top_issues.map((issue, i) => (
              <div key={i} className={`bg-white rounded-xl border shadow-sm p-4 ${issue.severity === 'critical' ? 'border-red-300 ring-1 ring-red-100 animate-[pulse-border_2s_ease-in-out_infinite]' : 'border-slate-100'}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      <span className="text-[10px] px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full">{issue.category}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${PRIORITY_BADGE[issue.severity] || PRIORITY_BADGE.medium}`}>
                        {issue.severity}
                      </span>
                      {issue.affected_count && (
                        <span className="text-[10px] px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full">~{issue.affected_count} affected</span>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {rd.recurring_themes.map((t, i) => (
                <span key={i} className="px-3 py-1.5 bg-purple-50 text-purple-700 rounded-full text-xs font-medium">{t}</span>
              ))}
            </div>
          </div>
        )}
      ...
      {rd.actionable_recommendations?.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-slate-700 mb-3">📌 Actionable Recommendations</h3>
          <div className="space-y-2">
            {rd.actionable_recommendations.map((rec, i) => (
              <div key={i} className="flex items-start gap-3 bg-white rounded-xl border border-slate-100 shadow-sm p-4 border-l-4 border-l-[#1a237e]">
...
function AllFeedbackTab({ role }) {
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    api.get(endpoint).then(r => setItems(r.data.suggestions || [])).catch(() => {}).finally(() => setLoading(false));
  }, [endpoint]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex flex-wrap gap-3 items-center">
        <FilterSelect label="Category" value={filter.category} onChange={v => setFilter(f => ({ ...f, category: v }))}
          options={CATEGORIES.map(c => ({ value: c.key, label: c.label }))} />
        <FilterSelect label="Sentiment" value={filter.sentiment} onChange={v => setFilter(f => ({ ...f, sentiment: v }))}
          options={['positive','neutral','negative','mixed'].map(v => ({ value: v, label: v }))} />
        <FilterSelect label="Priority" value={filter.priority} onChange={v => setFilter(f => ({ ...f, priority: v }))}
          options={['low','medium','high','critical'].map(v => ({ value: v, label: v }))} />
        <FilterSelect label="Status" value={filter.status} onChange={v => setFilter(f => ({ ...f, status: v }))}
          options={['pending','reviewed','actioned','dismissed'].map(v => ({ value: v, label: v }))} />
        <span className="text-xs text-slate-400 ml-auto">{filtered.length} results</span>
      </div>

      {!filtered.length && (
        <div className="bg-white rounded-xl border border-slate-100 p-8 text-center text-sm text-slate-400">No feedback matches filters.</div>
      )}
      ...
      {/* Respond Modal */}
      {respondModal && (
        <RespondModal suggestion={respondModal} onClose={() => { setRespondModal(null); fetchData(); }} />
      )}
    </div>
  );
}
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 hover:shadow-md transition-shadow">
      <div className="flex flex-wrap items-center gap-2 mb-2">
        ...
        {s.sentiment && <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${SENTIMENT_BADGE[s.sentiment] || ''}`}>{s.sentiment}</span>}
        {s.priority && <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${PRIORITY_BADGE[s.priority] || ''}`}>{s.priority}</span>}
        {s.status && <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${STATUS_BADGE[s.status] || ''}`}>{s.status}</span>}
        <span className="text-[10px] text-slate-400 ml-auto">{s.submitted_at ? new Date(s.submitted_at).toLocaleString() : ''}</span>
      </div>
      ...
      {showRespond && !s.admin_response && (
        <button onClick={onRespond} className="mt-2 text-xs px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200">
          💬 Respond
        </button>
      )}
    </div>
...
function RespondModal({ suggestion, onClose }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await api.patch(`/suggestions/${suggestion.id}/respond`, { admin_response: response.trim(), status });
      onClose();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 space-y-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="bg-slate-50 rounded-lg p-3 text-xs text-slate-600 max-h-32 overflow-y-auto">{suggestion.message}</div>
        {error && <p className="text-red-500 text-xs">{error}</p>}
        <textarea value={response} onChange={e => setResponse(e.target.value)} rows={4} placeholder="Write your official response..."
          className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 resize-none" />
    ...
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex flex-col items-center">
      <div className="relative w-16 h-16 mb-1">
        <div className="w-16 h-16 rounded-full"
          style={{ background: `conic-gradient(${color} ${score * 3.6}deg, #e2e8f0 ${score * 3.6}deg)` }}>
...
function SentimentBarCard({ breakdown, overall }) {
  const b = breakdown || {};
  const total = (b.positive_count || 0) + (b.neutral_count || 0) + (b.negative_count || 0) + (b.mixed_count || 0) || 1;
  const pct = v => Math.round((v || 0) / total * 100);
  return (
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
      <p className="text-[10px] text-slate-400 mb-2">Sentiment</p>
      <div className="flex h-4 rounded-full overflow-hidden mb-2">
        {pct(b.positive_count) > 0 && <div className="bg-emerald-400" style={{ width: `${pct(b.positive_count)}%` }} />}
        {pct(b.neutral_count) > 0 && <div className="bg-slate-300" style={{ width: `${pct(b.neutral_count)}%` }} />}
        {pct(b.negative_count) > 0 && <div className="bg-red-400" style={{ width: `${pct(b.negative_count)}%` }} />}
        {pct(b.mixed_count) > 0 && <div className="bg-amber-400" style={{ width: `${pct(b.mixed_count)}%` }} />}
      </div>
...
function CategoryBarChart({ data }) {
  ...
  const colors = ['bg-indigo-500','bg-blue-500','bg-emerald-500','bg-amber-500','bg-red-500','bg-purple-500','bg-cyan-500','bg-orange-500','bg-pink-500','bg-teal-500','bg-rose-500'];
  const catLabel = { teaching_quality:'Teaching Quality', infrastructure:'Infrastructure', syllabus:'Syllabus', administration:'Admin',
    canteen:'Canteen', hostel:'Hostel', sports:'Sports', library:'Library', other:'Other', class_environment:'Class Env', student_engagement:'Engagement' };
  return (
    <div className="space-y-2">
      {entries.map(([key, val], i) => (
        <div key={key} className="flex items-center gap-3">
          <span className="text-xs text-slate-600 w-28 text-right truncate">{catLabel[key] || key}</span>
