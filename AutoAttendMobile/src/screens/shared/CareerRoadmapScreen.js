...
export default function CareerRoadmapScreen() {
  ...
  const fetchSaved = useCallback(async () => {
    try {
      ...
      console.warn('[Career] saved fetch error:', err?.message);
    }
  }, []);
          ...
          {showForm ? (
            <View style={styles.card}>
              ...
              <TextInput
                ...
                placeholder="e.g. Backend Engineer at FAANG"
              ...
              >
                {generating ? (
                  ...
                  <>
                    ...
                    <Text style={styles.btnTxt}>Generate Roadmap</Text>
                  </>
                )}
              </TouchableOpacity>
            </View>
...
function RoadmapBlock({ data }) {
  ...
  if (!phases.length && data.summary) {
    return (
      <Text style={{ fontSize: 13, color: '#1e293b', lineHeight: 20 }}>{String(data.summary)}</Text>
    );
  }
  if (!phases.length) {
    return (
      <Text style={{ fontSize: 11, color: '#94a3b8' }} selectable>
  ...
  return (
    <View>
      {data.summary ? (
        <Text style={{ fontSize: 13, color: '#475569', marginBottom: 12, lineHeight: 20 }}>
          {String(data.summary)}
        </Text>
      ) : null}
      {phases.map((p, i) => (
        <View key={i} style={styles.phaseCard}>
          <View style={styles.phaseHead}>
            <View style={styles.phaseDot}>
              <Text style={styles.phaseDotTxt}>{i + 1}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.phaseTitle}>
                {p.title ?? p.name ?? p.phase ?? `Phase ${i + 1}`}
              </Text>
            ...
            <View style={styles.tags}>
              {p.skills.map((s, j) => (
                <View key={j} style={styles.tag}>
                  <Text style={styles.tagTxt}>{String(s)}</Text>
                </View>
              ))}
            </View>
          )}
