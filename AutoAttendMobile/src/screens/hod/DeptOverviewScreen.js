...
export default function DeptOverviewScreen() {
  ...
  if (!data) return <View style={styles.center}><Text style={styles.errTxt}>Failed to load data.</Text></View>;
    ...
    <SafeAreaView style={styles.safe} edges={['left', 'right']}>
      <ScrollView contentContainerStyle={styles.scroll}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.statsRow}>
          ...
          <StatCard icon="stats-chart-outline" label="Avg %" value={`${avgPct.toFixed(1)}%`} color={pctColor(avgPct)} />
          <StatCard icon="hourglass-outline" label="Pending" value={data.pending_approvals} color={data.pending_approvals > 0 ? '#ef4444' : '#22c55e'} />
        </ScrollView>
        ...
        {(data.subjects ?? []).map(s => (
          <View key={s.id} style={styles.card}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.cardMeta}>{s.code} · Sem {s.semester ?? '—'} · {s.teacher_name || 'Unassigned'}</Text>
            </View>
            <View style={styles.pctBadge}>
              <Text style={[styles.pctTxt, { color: pctColor(s.avg_pct) }]}>{s.avg_pct?.toFixed(0) ?? 0}%</Text>
...
const styles = StyleSheet.create({
  ...
  statCard: { width: 110, backgroundColor: '#fff', borderRadius: 14, padding: 14, marginRight: 10, alignItems: 'center', gap: 4, borderWidth: 1, borderColor: '#e2e8f0' },
