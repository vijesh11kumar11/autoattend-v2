...
export default function HODAlertsScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch (err) { console.warn('[HODAlerts]', err?.message); }
  }, []);
        ...
        <ScrollView contentContainerStyle={styles.scroll}
          ...
          {tab === 'Bulk' && (
            <View style={styles.card}>
              ...
              <TextInput style={[styles.input, { minHeight: 110, textAlignVertical: 'top' }]}
                value={bulkMsg} onChangeText={t => setBulkMsg(t.slice(0, MAX_LEN))}
                multiline placeholder="Will be sent to all defaulters" placeholderTextColor="#94a3b8" />
              <Text style={styles.cnt}>{bulkMsg.length}/{MAX_LEN}</Text>
              <TouchableOpacity style={[styles.bigBtn, sendingBulk && { opacity: 0.5 }]} onPress={sendBulk} disabled={sendingBulk}>
                {sendingBulk ? <ActivityIndicator color="#fff" /> :
                  <><Ionicons name="megaphone-outline" size={16} color="#fff" /><Text style={styles.bigBtnTxt}>Send Bulk Alert</Text></>}
              </TouchableOpacity>
            </View>
          )}
          ...
          {tab === 'Custom' && (
            <View style={styles.card}>
              ...
              <Text style={styles.label}>Message</Text>
              <TextInput style={[styles.input, { minHeight: 110, textAlignVertical: 'top' }]}
                value={customMsg} onChangeText={t => setCustomMsg(t.slice(0, MAX_LEN))}
                multiline placeholder="Personal message" placeholderTextColor="#94a3b8" />
              <Text style={styles.cnt}>{customMsg.length}/{MAX_LEN}</Text>

              <TouchableOpacity style={[styles.bigBtn, sendingCustom && { opacity: 0.5 }]} onPress={sendCustom} disabled={sendingCustom}>
                {sendingCustom ? <ActivityIndicator color="#fff" /> :
                  <><Ionicons name="send-outline" size={16} color="#fff" /><Text style={styles.bigBtnTxt}>Send Custom Alert</Text></>}
              </TouchableOpacity>
            </View>
          )}
          ...
          {tab === 'History' && (
            <>
              ...
              {history.length === 0 ? <Text style={styles.empty}>No history.</Text> :
                history.map((a, i) => (
                  <View key={a.id ?? i} style={styles.histCard}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.histMeta}>
                        {(a.recipient_name ?? a.student_name ?? 'Bulk')} · {timeAgo(a.created_at ?? a.sent_at)}
                      </Text>
                    </View>
                    <View style={[styles.statusPill, { backgroundColor: a.status === 'sent' ? '#dcfce7' : '#fef3c7' }]}>
      ...
      <Modal visible={pickerOpen} animationType="slide" transparent onRequestClose={() => setPickerOpen(false)}>
        <View style={styles.modalBg}>
          <View style={[styles.modalCard, { maxHeight: '85%' }]}>
            ...
            <TextInput style={styles.input} value={pickerSearch} onChangeText={setPickerSearch}
              placeholder="Search…" placeholderTextColor="#94a3b8" autoFocus />
            <ScrollView style={{ marginTop: 8 }}>
              {filteredStudents.slice(0, 200).map(s => (
                <TouchableOpacity key={s.id} style={styles.pickRow} onPress={() => { setPicked(s); setPickerOpen(false); setPickerSearch(''); }}>
                  <View style={{ flex: 1 }}>
            ...
            <TouchableOpacity onPress={() => setPickerOpen(false)} style={{ marginTop: 8, alignItems: 'center' }}>
...
const styles = StyleSheet.create({
  ...
  bigNum: { fontSize: 36, fontWeight: '800', color: '#ef4444', textAlign: 'center', paddingVertical: 8 },
  ...
  histCard: { backgroundColor: '#fff', borderRadius: 10, padding: 12, marginBottom: 6, borderWidth: 1, borderColor: '#e2e8f0', flexDirection: 'row', alignItems: 'flex-start' },
  ...
  pickRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
