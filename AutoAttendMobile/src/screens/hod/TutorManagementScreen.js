...
export default function TutorManagementScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((a) =>
      `${a.tutor_name} ${a.student_name} ${a.student_roll}`.toLowerCase().includes(q)
    );
  }, [items, search]);

  const remove = (a) =>
    Alert.alert('Remove Assignment', `Remove ${a.student_name} from ${a.tutor_name}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await client.delete(`/tutor/remove/${a.id}`);
            fetchData();
          } catch (err) {
  ...
  const submitBulk = async () => {
    ...
    try {
      if (mode === 'range') {
        if (!form.roll_start.trim() || !form.roll_end.trim())
          throw new Error('Roll range required.');
      ...
      setShowBulk(false);
      setForm({ tutor_id: '', roll_start: '', roll_end: '', section_id: '' });
      fetchData();
    } catch (err) {
      ...
      >
        {tab === 'Assignments' && (
          <>
            ...
            {filtered.length === 0 ? (
              <Text style={styles.empty}>No assignments.</Text>
            ) : (
              filtered.map((a) => (
                <View key={a.id} style={styles.card}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.title}>
                      {a.student_name} <Text style={styles.meta}>({a.student_roll})</Text>
                    </Text>
                    <Text style={styles.meta}>Tutor: {a.tutor_name}</Text>
                    <Text style={styles.meta}>
                      {a.section_name ?? '—'} · AY {a.academic_year}
                    </Text>
                  </View>
        ...
        {tab === 'Unassigned' && (
          <>
            ...
            {unassigned.length === 0 ? (
              <Text style={styles.empty}>All assigned.</Text>
            ) : (
              unassigned.map((s) => (
        ...
        {tab === 'Bulk Assign' && (
          <View style={styles.bulkCard}>
            ...
            <TextInput
              style={styles.input}
              value={form.tutor_id}
              onChangeText={(t) => setForm({ ...form, tutor_id: t })}
              keyboardType="number-pad"
            ...
            {mode === 'range' ? (
              <>
                ...
                <TextInput
                  ...
                  onChangeText={(t) => setForm({ ...form, roll_start: t })}
                ...
                <TextInput
                  style={styles.input}
                  value={form.roll_end}
                  onChangeText={(t) => setForm({ ...form, roll_end: t })}
                  placeholder="e.g. 21CSE060"
                  placeholderTextColor="#94a3b8"
                />
              </>
            ) : (
              <>
                ...
                <TextInput
                  ...
                  onChangeText={(t) => setForm({ ...form, section_id: t })}
                  keyboardType="number-pad"
                />
              </>
            )}
            <TouchableOpacity
              style={[styles.bigBtn, busy && { opacity: 0.5 }]}
              onPress={submitBulk}
              disabled={busy}
            >
              {busy ? (
                ...
                <>
                  ...
                  <Text style={styles.bigBtnTxt}>Assign Bulk</Text>
                </>
              )}
            </TouchableOpacity>
