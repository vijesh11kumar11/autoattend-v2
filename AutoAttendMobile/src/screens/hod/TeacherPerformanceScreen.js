...
export default function TeacherPerformanceScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch (err) { console.warn('[TeacherPerf]', err?.message); }
  }, []);
  ...
  const view = useMemo(() => {
    ...
    return [...r].sort((a, b) => {
      switch (sort) {
        ...
        case 'subjects':   return (b.subjects_count ?? 0) - (a.subjects_count ?? 0);
        case 'sessions':   return (b.sessions_conducted_this_month ?? 0) - (a.sessions_conducted_this_month ?? 0);
        case 'disputes':   return (b.pending_disputes ?? 0) - (a.pending_disputes ?? 0);
        default:           return (b.avg_attendance_pct ?? 0) - (a.avg_attendance_pct ?? 0);
      }
    });
  }, [items, search, sort]);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <TextInput style={styles.search} value={search} onChangeText={setSearch}
        placeholder="Search teacher" placeholderTextColor="#94a3b8" />

      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.sortStrip}
      ...
      <ScrollView contentContainerStyle={styles.scroll}
        ...
        {view.length === 0 ? <Text style={styles.empty}>No teachers.</Text> :
          view.map(t => (
            <View key={t.teacher_id ?? t.id} style={styles.card}>
              <View style={{ flex: 1 }}>
                ...
                <View style={styles.metricsRow}>
                  ...
                  <Metric label="Disputes" value={t.pending_disputes ?? 0}
                    color={(t.pending_disputes ?? 0) > 0 ? '#ef4444' : null} />
                </View>
              </View>
              <View style={[styles.pctBox, { backgroundColor: pctColor(t.avg_attendance_pct ?? 0) }]}>
...
const styles = StyleSheet.create({
  ...
  search: { backgroundColor: '#fff', marginHorizontal: 16, marginBottom: 8, borderRadius: 10, padding: 10, fontSize: 13, color: '#1e293b', borderWidth: 1, borderColor: '#e2e8f0' },
