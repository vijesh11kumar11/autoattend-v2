...
export default function SubjectsScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('[Subjects] error:', err?.message);
    }
  }, []);
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      await client.patch(`/hod/subjects/${editing.id}/total-lectures`, null, {
        params: { total_lectures: n },
      });
      setEditing(null);
      setVal('');
      fetchData();
            ...
            <View key={s.id} style={styles.card}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.meta}>
                  {s.code} · {s.course_name ?? `Course ${s.course_id}`} · Sem {s.semester}
                </Text>
                <Text style={styles.totalLec}>
                  Total Lectures:{' '}
                  <Text style={{ fontWeight: '800', color: PRIMARY }}>
                    {s.total_lectures ?? '—'}
                  </Text>
                </Text>
              </View>
              <TouchableOpacity
                ...
                onPress={() => {
                  setEditing(s);
                  setVal(String(s.total_lectures ?? ''));
                }}
              >
