...
export default function SectionsScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('[Sections] error:', err?.message);
    }
  }, [filterSem]);
  ...
  const openNew = () => {
    setForm({ course_id: '', semester: '', name: '', max_strength: '60' });
    setEditing('new');
  };
  const openEdit = (row) => {
    setForm({
      ...
      name: row.name,
      max_strength: String(row.max_strength ?? 60),
    });
    setEditing(row);
  };
  ...
  const remove = (row) =>
    Alert.alert('Delete Section', `Delete "${row.name}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await client.delete(`/sections/${row.id}`);
            fetchData();
          } catch (err) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
            ...
            <View key={s.id} style={styles.card}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.meta}>
                  {s.course_name ?? `Course ${s.course_id}`} · Cap {s.max_strength}
                </Text>
              </View>
              <TouchableOpacity onPress={() => viewStudents(s)}>
                <Ionicons name="people-outline" size={20} color={PRIMARY} />
              </TouchableOpacity>
              <TouchableOpacity onPress={() => openEdit(s)} style={{ marginLeft: 12 }}>
                <Ionicons name="create-outline" size={20} color="#3b82f6" />
              </TouchableOpacity>
              <TouchableOpacity onPress={() => remove(s)} style={{ marginLeft: 12 }}>
        ...
        <View style={styles.modalBg}>
          <View style={styles.modalCard}>
            ...
            {editing === 'new' && (
              <>
                ...
                <TextInput
                  ...
                  onChangeText={(t) => setForm({ ...form, course_id: t })}
                  keyboardType="number-pad"
                />
            ...
            <TextInput
              ...
              onChangeText={(t) => setForm({ ...form, max_strength: t })}
              keyboardType="number-pad"
            />
      ...
      <Modal
        ...
        animationType="slide"
        transparent
        onRequestClose={() => setMembers(null)}
      >
        <View style={styles.modalBg}>
