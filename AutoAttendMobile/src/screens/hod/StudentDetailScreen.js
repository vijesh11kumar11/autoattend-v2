...
export default function StudentDetailScreen({ route }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch (err) { console.warn('[StudentDetail] fetch error:', err?.message); }
  }, [student_id]);
    ...
    <SafeAreaView style={styles.safe} edges={['left', 'right']}>
      <ScrollView contentContainerStyle={styles.scroll}
        ...
        <View style={styles.headerCard}>
          <View style={styles.avatar}><Text style={styles.avatarTxt}>{(student_name ?? summary?.name ?? '?')[0]?.toUpperCase()}</Text></View>
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={styles.name}>{student_name ?? summary?.name ?? `Student #${student_id}`}</Text>
              ...
              return (
                <View key={s.subject_id ?? i} style={styles.subCard}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.subMeta}>{s.present ?? 0}/{s.total ?? 0} sessions</Text>
                  </View>
          ...
          : recent.slice(0, 30).map((r, i) => (
              <View key={i} style={styles.recRow}>
                ...
                <View style={{ flex: 1, marginLeft: 8 }}>
                  ...
                  <Text style={styles.recMeta}>{r.date ?? r.session_date ?? r.created_at?.slice(0, 10)}</Text>
                </View>
              </View>
            ))}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  statusChip: { alignSelf: 'flex-start', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8, marginTop: 6 },
  statusTxt: { fontSize: 10, fontWeight: '700' },
  pctCircle: { width: 60, height: 60, borderRadius: 30, backgroundColor: '#f1f5f9', alignItems: 'center', justifyContent: 'center' },
  ...
  recRow: { backgroundColor: '#fff', borderRadius: 10, padding: 10, marginBottom: 4, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#e2e8f0' },
