...
export default function HODReportsHomeScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (sub.status === 'fulfilled') setSessions(Array.isArray(sub.value.data) ? sub.value.data : []);
      if (stu.status === 'fulfilled') setStudents(Array.isArray(stu.value.data) ? stu.value.data : []);
    } catch (err) { console.warn('[HODReports] err:', err?.message); }
  }, [subjectId]);

  useEffect(() => { fetchData().finally(() => setLoading(false)); }, [fetchData]);
  const onRefresh = useCallback(async () => { setRefreshing(true); await fetchData(); setRefreshing(false); }, [fetchData]);

  const dlDefaultersPdf = () => downloadAndShare({
    path: '/api/reports/hod/defaulters/pdf',
    fileName: `defaulters_${new Date().toISOString().slice(0, 10)}.pdf`,
    fallbackExt: 'pdf',
    title: 'Defaulters Report',
  });
  const dlSessionPdf = sid => downloadAndShare({
    path: `/api/reports/class/${sid}/pdf`,
    fileName: `session_${sid}.pdf`,
    fallbackExt: 'pdf',
    title: `Session ${sid}`,
  });
  const dlStudentPdf = stu => downloadAndShare({
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll}
        ...
        <TextInput style={styles.input} value={subjectId} onChangeText={setSubjectId}
          placeholder="Filter by subject ID" placeholderTextColor="#94a3b8" keyboardType="number-pad"
          onSubmitEditing={fetchData} />

        {sessions.length === 0 ? <Text style={styles.empty}>No sessions.</Text> :
          sessions.slice(0, 100).map(s => (
            <View key={s.id} style={styles.card}>
              <View style={{ flex: 1 }}>
                <Text style={styles.title}>{s.subject_name ?? `Session ${s.id}`}</Text>
                <Text style={styles.meta}>{s.date} · {s.present ?? 0}/{s.total ?? 0} present</Text>
              </View>
        ...
        {students.length > 100 && <Text style={styles.note}>Showing 100 of {students.length}…</Text>}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  bigBtn: { flexDirection: 'row', backgroundColor: '#ef4444', padding: 14, borderRadius: 10, alignItems: 'center', justifyContent: 'center', gap: 8 },
  bigBtnTxt: { color: '#fff', fontWeight: '800' },
  input: { backgroundColor: '#fff', borderRadius: 10, padding: 12, borderWidth: 1, borderColor: '#e2e8f0', marginBottom: 10, fontSize: 13, color: '#1e293b' },
