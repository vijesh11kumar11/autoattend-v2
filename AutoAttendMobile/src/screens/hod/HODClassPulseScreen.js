...
export default function HODClassPulseScreen({ route }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setData(d);
    } catch (err) { console.warn('[HODClassPulse]', err?.message); }
  }, [departmentId]);
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll}
        ...
        <View style={styles.statsGrid}>
          ...
          <Stat label="Avg Engagement" value={`${(stats.avg_engagement_pct ?? 0).toFixed(0)}%`}
            color={pctColor(stats.avg_engagement_pct ?? 0)} icon="pulse-outline" />
          <Stat label="Avg Comprehension" value={`${(stats.avg_comprehension_pct ?? 0).toFixed(0)}%`}
            color={pctColor(stats.avg_comprehension_pct ?? 0)} icon="bulb-outline" />
          <Stat label="Students at Risk" value={stats.students_at_risk_count ?? 0}
            color="#ef4444" icon="warning-outline" />
        </View>

        <Text style={styles.section}>Subjects</Text>
        {(data?.subjects_overview ?? []).length === 0 ? <Text style={styles.empty}>No subjects.</Text> :
          (data?.subjects_overview ?? []).map(s => (
            <TouchableOpacity key={s.subject_id ?? s.id} style={styles.card} onPress={() => openSubject(s.subject_id ?? s.id)}>
              <View style={{ flex: 1 }}>
          ...
          <>
            ...
            {data.teachers_not_using.map((t, i) => (
              <View key={i} style={styles.notUsingRow}>
                ...
                <Text style={styles.notUsingTxt}>{t.name ?? t.teacher_name ?? `Teacher ${t.id}`}</Text>
              </View>
            ))}
          </>
        )}
      ...
      <Modal visible={!!report} animationType="slide" transparent onRequestClose={() => setReport(null)}>
                ...
                <>
                  ...
                  {report.recent_capsules.slice(0, 10).map((c, i) => (
                    <View key={i} style={styles.miniCard}>
                      <Text style={{ fontSize: 12, fontWeight: '700', color: '#1e293b' }}>{c.title ?? `Capsule ${c.id}`}</Text>
                      <Text style={styles.meta}>Eng {(c.engagement_pct ?? 0).toFixed(0)}% · Comp {(c.comprehension_pct ?? 0).toFixed(0)}%</Text>
                    </View>
                  ))}
                </>
              )}
              ...
              {report?.knowledge_gaps?.length > 0 && (
                <>
                  ...
                  {report.knowledge_gaps.slice(0, 10).map((g, i) => (
                    <Text key={i} style={[styles.doubtTxt, { marginBottom: 4 }]}>• {typeof g === 'string' ? g : (g.topic ?? JSON.stringify(g))}</Text>
                  ))}
                </>
              )}

              <TouchableOpacity style={[styles.bigBtn, { marginTop: 14 }]} onPress={() => setReport(null)}>
...
const styles = StyleSheet.create({
  ...
  statBox: { width: '48%', backgroundColor: '#fff', borderRadius: 10, padding: 14, alignItems: 'center', gap: 4, borderWidth: 1, borderColor: '#e2e8f0' },
  ...
  notUsingRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fef2f2', padding: 10, borderRadius: 8, marginBottom: 4, gap: 8 },
