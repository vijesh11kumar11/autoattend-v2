...
export default function StudentsScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('[StudentsScreen] fetch error:', err?.message);
    }
  }, []);
        ...
        ListHeaderComponent={
          <View>
            ...
            <Text style={styles.sub}>
              {studentCount} students across {subjects.length} subjects
            </Text>
            <TextInput
              style={styles.search}
        ...
        renderItem={({ item: s }) => (
          <View style={styles.card}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.meta}>
                {s.teacher_name || 'Unassigned'} · {s.sessions_done} sessions
              </Text>
            </View>
            <View style={styles.pctCol}>
              <Text style={[styles.pctVal, { color: pctColor(s.avg_pct) }]}>
              ...
              <View style={styles.bar}>
                <View
                  style={[
                    styles.barFill,
                    {
                      width: `${Math.min(s.avg_pct ?? 0, 100)}%`,
                      backgroundColor: pctColor(s.avg_pct),
                    },
                  ]}
                />
              </View>
