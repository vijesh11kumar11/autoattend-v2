...
export default function SemesterProgressScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setData(d);
    } catch (err) { console.warn('[SemesterProgress] fetch error:', err?.message); }
  }, []);
  ...
  if (!data)   return <View style={styles.center}><Text style={styles.err}>Failed to load.</Text></View>;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['left', 'right']}>
      <ScrollView contentContainerStyle={styles.scroll}
        ...
        {weeks != null && (
          <Text style={styles.sub}>Week {weeks}{totalWeeks ? ` of ${totalWeeks}` : ''}</Text>
        )}

        <View style={styles.overallCard}>
          <Text style={styles.overallLabel}>Overall Sessions Completed</Text>
          <Text style={[styles.overallVal, { color: pctColor(overall) }]}>{Number(overall).toFixed(1)}%</Text>
          <View style={styles.bar}>
            <View style={[styles.fill, { width: `${Math.min(overall, 100)}%`, backgroundColor: pctColor(overall) }]} />
          </View>
        </View>
              ...
              return (
                <View key={s.id ?? i} style={styles.card}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.meta}>{s.code ?? ''} · {s.sessions_done ?? s.completed ?? 0}/{s.total_sessions ?? s.total ?? '?'} sessions</Text>
                    <View style={styles.bar}>
...
const styles = StyleSheet.create({
  ...
  overallCard: { backgroundColor: '#fff', borderRadius: 14, padding: 16, marginBottom: 20, borderWidth: 1, borderColor: '#e2e8f0' },
