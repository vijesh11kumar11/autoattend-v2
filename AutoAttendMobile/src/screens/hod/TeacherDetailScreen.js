...
export default function TeacherDetailScreen({ route }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('[TeacherDetail] fetch error:', err?.message);
    }
  }, []);
      ...
      >
        <View style={styles.headerCard}>
          <View style={styles.avatar}>
            <Text style={styles.avatarTxt}>
              {(teacher.name ?? teacher_name ?? '?')[0]?.toUpperCase()}
            </Text>
          </View>
        ...
        <View style={styles.statsRow}>
          <View style={styles.stat}>
            ...
            <Text style={styles.statV}>{teacher.subject_count ?? subjects.length ?? 0}</Text>
            <Text style={styles.statL}>Subjects</Text>
          </View>
          <View style={styles.stat}>
            <Ionicons name="calendar-outline" size={20} color={PRIMARY} />
            <Text style={styles.statV}>{teacher.total_sessions ?? teacher.sessions ?? 0}</Text>
            <Text style={styles.statL}>Sessions</Text>
          </View>
          <View style={styles.stat}>
            <Ionicons name="stats-chart-outline" size={20} color={pctColor(avg)} />
            <Text style={[styles.statV, { color: pctColor(avg) }]}>{Number(avg).toFixed(1)}%</Text>
            ...
            <View key={s.id ?? i} style={styles.subCard}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.subMeta}>
                  {s.code ?? ''} · {s.sessions ?? s.sessions_done ?? 0} sessions
                </Text>
              </View>
              <Text style={[styles.subPct, { color: pctColor(s.avg_pct ?? 0) }]}>
                {Number(s.avg_pct ?? 0).toFixed(0)}%
              </Text>
            </View>
          ))
        )}
