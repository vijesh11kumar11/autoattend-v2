...
export default function HODTimetableScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('[Timetable] error:', err?.message);
    }
  }, []);
  ...
  const fetchOptions = useCallback(async () => {
    try {
      ...
      console.warn('[Timetable] options error:', err?.message);
    }
  }, []);

  useEffect(() => {
    Promise.all([fetchData(), fetchOptions()]).finally(() => setLoading(false));
  ...
  const removeEntry = (e) =>
    Alert.alert('Delete Entry', `Delete ${e.subject_name} ${e.start_time}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await client.delete(`/timetable/entry/${e.id}`);
            fetchData();
          } catch (err) {
          ...
          >
            ...
            {byDay[d]?.length > 0 && (
              <View style={styles.cnt}>
                <Text style={styles.cntTxt}>{byDay[d].length}</Text>
              </View>
            )}
          </TouchableOpacity>
        ))}
              ...
              >
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.subMeta}>
                    {e.teacher_name ?? '—'} · {e.section_name ?? ''} · Room {e.room ?? '—'}
                  </Text>
                </View>
          ...
          <View style={styles.modalCard}>
            ...
            <ScrollView style={{ maxHeight: 460 }} keyboardShouldPersistTaps="handled">
              ...
              <View style={styles.chipWrap}>
                {subjects.length === 0 ? (
                  <Text style={styles.hint}>No subjects available.</Text>
                ) : (
                  subjects.map((s) => {
              ...
              <View style={styles.chipWrap}>
                {teachers.length === 0 ? (
                  <Text style={styles.hint}>No teachers available.</Text>
                ) : (
                  teachers.map((t) => {
              ...
              <View style={styles.chipWrap}>
                <TouchableOpacity
                  style={[styles.chip, !form.section_id && styles.chipSel]}
              ...
              <View style={styles.switchRow}>
                ...
                <Switch
                  value={form.is_lab}
                  onValueChange={(v) => setForm((f) => ({ ...f, is_lab: v }))}
