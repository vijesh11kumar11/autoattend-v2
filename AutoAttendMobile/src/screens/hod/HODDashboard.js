...
export default function HODDashboard({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('[HODDashboard] fetch error:', err?.message);
    }
  }, []);
  ...
  const statCards = [
    ...
    { icon: 'school-outline', label: 'Students', value: stats.students, color: '#8b5cf6' },
    {
      ...
      color: stats.avg >= 75 ? '#22c55e' : '#f59e0b',
    },
          ...
          {[
            {
              ...
              onPress: () => navigation.navigate('SemesterProgress'),
            },
            {
              icon: 'grid-outline',
              label: 'Section Analytics',
              onPress: () => navigation.navigate('SectionAnalytics'),
            },
            {
              icon: 'pie-chart-outline',
              label: 'Dept Overview',
              onPress: () => navigation.navigate('DeptOverview'),
            },
          ].map((a, i) => (
            <TouchableOpacity
              key={i}
        ...
        <View style={styles.actionsGrid}>
          {[
            ...
            },
            {
              icon: 'calendar-outline',
              label: 'Timetable',
              onPress: () => navigation.navigate('HODTimetable'),
            },
            {
          ...
          {[
            {
              ...
              label: 'Tutors',
              onPress: () => navigation.navigate('TutorManagement'),
            },
            {
              icon: 'document-outline',
              label: 'Reports Hub',
              onPress: () => navigation.navigate('HODReportsHome'),
            },
            {
              icon: 'radio-outline',
              label: 'Live Sessions',
              onPress: () => navigation.navigate('LiveAnalytics'),
            },
          ].map((a, i) => (
            <TouchableOpacity
              key={i}
        ...
        <View style={[styles.actionsGrid, { marginTop: 10 }]}>
          {[
            ...
            },
            {
              icon: 'shield-outline',
              label: 'Disputes',
              onPress: () => navigation.navigate('HODDisputes'),
            },
            {
              icon: 'film-outline',
              label: 'ClassPulse',
              onPress: () => navigation.navigate('HODClassPulse'),
            },
          ].map((a, i) => (
            <TouchableOpacity
              key={i}
