...
export default function HODDisputesScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('[Disputes]', err?.message);
    }
  }, []);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        {items.length === 0 ? (
          <Text style={styles.empty}>No pending disputes.</Text>
        ) : (
          items.map((d) => (
            <TouchableOpacity key={d.id} style={styles.card} onPress={() => setActive(d)}>
              <View style={{ flex: 1 }}>
                <Text style={styles.title}>
                  {d.student_name} <Text style={styles.meta}>({d.roll_number})</Text>
                </Text>
                <Text style={styles.meta}>
                  {d.subject_name} · {d.session_date}
                </Text>
                ...
                {d.resolved_by && (
                  <Text style={styles.meta}>
                    Resolved by: {d.resolved_by} — {d.resolution_note}
                  </Text>
                )}
              </View>
              <View
                style={[
                  styles.pill,
                  { backgroundColor: d.status === 'pending' ? '#fde68a' : '#dcfce7' },
                ]}
              >
      ...
      >
        <View style={styles.modalBg}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.meta}>
              {active?.subject_name} · {active?.session_date}
            </Text>
            <View style={styles.reasonBox}>
              <Text style={styles.reasonTxt}>{active?.reason}</Text>
            </View>
