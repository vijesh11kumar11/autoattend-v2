...
export default function PrincipalAuditScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch (err) { console.warn('[PrincipalAudit] fetch error:', err?.message); }
  }, []);

  useEffect(() => { fetchData().finally(() => setLoading(false)); }, [fetchData]);
  const onRefresh = useCallback(async () => {
    setRefreshing(true); setPage(50); await fetchData(); setRefreshing(false);
  }, [fetchData]);
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter(l =>
      String(l.action ?? l.event ?? '').toLowerCase().includes(q) ||
      String(l.performed_by ?? l.user ?? '').toLowerCase().includes(q) ||
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <FlatList
        ...
        onEndReached={() => { if (slice.length < filtered.length) setPage(p => p + 50); }}
        ListHeaderComponent={
          <View>
          ...
          <View style={styles.card}>
            ...
            <View style={{ flex: 1, marginLeft: 8 }}>
              ...
              <Text style={styles.meta}>
                {item.performed_by ?? item.user ?? 'system'} · {timeAgo(item.created_at ?? item.timestamp)}
              </Text>
            </View>
          </View>
        )}
        ListFooterComponent={slice.length < filtered.length ? (
...
const styles = StyleSheet.create({
  ...
  search: { backgroundColor: '#fff', borderRadius: 10, padding: 12, fontSize: 14, color: '#1e293b', borderWidth: 1, borderColor: '#e2e8f0', marginBottom: 12 },
  card: { backgroundColor: '#fff', borderRadius: 12, padding: 12, marginBottom: 8, flexDirection: 'row', alignItems: 'flex-start', borderWidth: 1, borderColor: '#e2e8f0' },
