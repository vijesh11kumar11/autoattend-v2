...
export default function PrincipalClassPulseScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('[PrincipalCP]', err?.message);
    }
  }, []);
  ...
  const openDept = async (d) => {
    ...
    setAnalytics(null);
    try {
      const { data } = await client.get('/classpulse/hod/department-analytics', {
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        {departments.length === 0 ? (
          <Text style={styles.empty}>No departments.</Text>
        ) : (
          departments.map((d) => (
            <TouchableOpacity
              ...
              style={styles.card}
              onPress={() =>
                openDept({ id: d.id ?? d.department_id, name: d.name ?? d.department_name })
              }
            >
              <View style={{ flex: 1 }}>
                <Text style={styles.title}>{d.name ?? d.department_name}</Text>
                <Text style={styles.meta}>
                  {d.total_students ?? 0} students · {d.total_teachers ?? 0} teachers · Avg{' '}
                  {(d.avg_attendance ?? d.attendance_pct ?? 0).toFixed(0)}%
                </Text>
              </View>
            ...
            {busy ? (
              ...
              <ScrollView>
                <View style={styles.statsGrid}>
                  <Stat
                    label="Capsules"
                    value={analytics?.department_stats?.total_capsules ?? 0}
                    icon="film-outline"
                  />
                  <Stat
                    label="Engagement"
                    value={`${(analytics?.department_stats?.avg_engagement_pct ?? 0).toFixed(0)}%`}
                    color={pctColor(analytics?.department_stats?.avg_engagement_pct ?? 0)}
                    icon="pulse-outline"
                  />
                  <Stat
                    label="Comprehension"
                    value={`${(analytics?.department_stats?.avg_comprehension_pct ?? 0).toFixed(0)}%`}
                    color={pctColor(analytics?.department_stats?.avg_comprehension_pct ?? 0)}
                    icon="bulb-outline"
                  />
                  <Stat
                    label="At Risk"
                    value={analytics?.department_stats?.students_at_risk_count ?? 0}
                    color="#ef4444"
                  ...
                  <>
                    ...
                    {analytics.subjects_overview.slice(0, 20).map((s) => (
                      <View key={s.subject_id ?? s.id} style={styles.miniCard}>
                        ...
                        <Text style={styles.meta}>
                          {s.capsules_count ?? 0} capsules · Eng{' '}
                          {(s.avg_engagement ?? 0).toFixed(0)}% · Comp{' '}
                          {(s.avg_comprehension ?? 0).toFixed(0)}%
                        </Text>
                      </View>
                    ))}
                  </>
