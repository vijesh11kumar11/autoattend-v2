...
export default function PrincipalDashboard({ navigation }) {
  ...
  if (loading) {
    return <SafeAreaView style={styles.root}><StatsSkeleton count={5} /><CardSkeleton count={4} /></SafeAreaView>;
  }
    ...
    <SafeAreaView style={styles.root} edges={['left', 'right']}>
      ...
      >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.statsRow}>
          {(stats.length ? stats : [
            { label: 'Departments', value: departments.length, icon: 'business-outline', color: '#3b82f6' },
            { label: 'Students',    value: college.student_count ?? 0, icon: 'school-outline', color: '#8b5cf6' },
            { label: 'Teachers',    value: college.teacher_count ?? 0, icon: 'people-outline', color: '#06b6d4' },
            { label: 'Avg %',       value: `${(college.avg_percentage ?? 0).toFixed(1)}%`, icon: 'stats-chart-outline', color: pctColor(college.avg_percentage ?? 0) },
            { label: 'Alerts',      value: college.alert_count ?? 0,  icon: 'notifications-outline', color: '#ef4444' },
          ]).map((s, i) => (
