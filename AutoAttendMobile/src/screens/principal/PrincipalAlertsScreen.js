...
export default function PrincipalAlertsScreen() {
  ...
  const onRefresh = useCallback(async () => { setRefreshing(true); await fetchAudits(); setRefreshing(false); }, [fetchAudits]);
          ...
          ListHeaderComponent={
            <View>
              ...
              <View style={styles.sendCard}>
                ...
                <TextInput style={styles.textarea} placeholder="Type your alert message…"
                  placeholderTextColor="#94a3b8" value={message} onChangeText={t => setMessage(t.slice(0, MAX_LEN))}
                ...
                <View style={styles.sendRow}>
                  ...
                  <TouchableOpacity style={[styles.sendBtn, !message.trim() && styles.sendBtnDisabled]}
                    onPress={sendAlert} disabled={sending || !message.trim()}>
                    {sending
                      ? <ActivityIndicator size="small" color="#fff" />
                      : <><Ionicons name="send" size={16} color="#fff" /><Text style={styles.sendBtnTxt}> Send</Text></>}
                  </TouchableOpacity>
                </View>
              </View>
          ...
          renderItem={({ item: log }) => (
            <View style={styles.logCard}>
              ...
              <View style={{ flex: 1, marginLeft: 8 }}>
                ...
                <Text style={styles.logMeta}>{log.performed_by ?? log.user ?? ''} · {timeAgo(log.created_at ?? log.timestamp)}</Text>
              </View>
            </View>
          )}
        />
...
const styles = StyleSheet.create({
  ...
  sendBtn: { backgroundColor: PRIMARY, borderRadius: 10, paddingHorizontal: 16, paddingVertical: 10, flexDirection: 'row', alignItems: 'center' },
  sendBtnDisabled: { opacity: 0.5 },
  sendBtnTxt: { color: '#fff', fontWeight: '700', fontSize: 14 },
  section: { fontSize: 16, fontWeight: '700', color: '#1e293b', marginBottom: 12 },
  logCard: { backgroundColor: '#fff', borderRadius: 12, padding: 12, marginBottom: 8, flexDirection: 'row', alignItems: 'flex-start', borderWidth: 1, borderColor: '#e2e8f0' },
