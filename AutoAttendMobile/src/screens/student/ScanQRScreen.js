...
export default function ScanQRScreen({ navigation }) {
  ...
  // ─── POST face/verify ────────────────────────────────────────────────────
  const submitFaceVerify = useCallback(
    async (photoUri) => {
  ...
  const submitAttendance = useCallback(
    async (qrData) => {
      ...
      try {
        parsedSession = JSON.parse(qrData)?.session_id ?? qrData;
      } catch {
        parsedSession = qrData;
      }
      ...
      try {
        ...
        if (detail.includes('expired') || detail.includes('qr')) {
          ...
          detail.includes('distance') ||
          detail.includes('location') ||
          detail.includes('away')
        ) {
  ...
  if (screen === STATES.SUCCESS) {
    const time = successData?.time
      ? new Date(successData.time).toLocaleTimeString('en-IN', {
          hour: '2-digit',
          minute: '2-digit',
        })
          ...
          <View style={styles.qrTopBar}>
            ...
            <View style={styles.tokenTtlBadge}>
              <Ionicons
                name="timer-outline"
                size={14}
                color={faceTokenTTL <= 10 ? '#f87171' : '#fbbf24'}
              />
        ...
        <SafeAreaView style={styles.faceOverlay}>
          ...
          <View style={styles.faceBottom}>
            {faceCapturing ? (
              ...
              <>
                <Text style={styles.faceAutoText}>Auto-capturing in {faceCapCountdown}s</Text>
                <TouchableOpacity
  ...
  return (
    <SafeAreaView style={styles.idleContainer}>
      ...
      <View style={[styles.section, styles.bleCard]}>
        ...
        <View
          style={[styles.bleDot, { backgroundColor: beaconDetected ? '#22c55e' : '#e2e8f0' }]}
        />
      </View>

      {/* GPS status */}
      <View style={[styles.section, styles.bleCard]}>
        <Ionicons name="locate-outline" size={20} color={gpsCoords ? '#3b82f6' : '#94a3b8'} />
        <View style={{ flex: 1, marginLeft: 10 }}>
          <Text style={styles.bleTitle}>
            {gpsCoords
        ...
        <View style={subStyles.failedActions}>
          {cfg.actions.map((a, i) => (
            <TouchableOpacity
              ...
              style={[
                subStyles.failedBtn,
                a.primary ? subStyles.failedBtnPrimary : subStyles.failedBtnOutline,
              ]}
              onPress={a.onPress}
              activeOpacity={0.85}
            >
