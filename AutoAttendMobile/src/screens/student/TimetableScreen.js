  ...
  const renderItem = ({ item: s, index }) => (
    <View style={styles.card}>
      <View style={styles.cardRow}>
        <View style={styles.numBadge}><Text style={styles.numTxt}>{index + 1}</Text></View>
        <View style={{ flex: 1 }}>
          <Text style={styles.subName}>{s.subject_name}</Text>
          <Text style={styles.subCode}>{s.subject_code ?? ''} · Sem {s.semester ?? '—'}</Text>
        </View>
        <Text style={[styles.pct, { color: pctColor(s.percentage) }]}>{s.percentage}%</Text>
      </View>
      <View style={styles.bar}><View style={[styles.barFill, { width: `${Math.min(s.percentage, 100)}%`, backgroundColor: pctColor(s.percentage) }]} /></View>
      <View style={styles.metaRow}>
        <Text style={styles.metaTxt}>{s.present}/{s.total_sessions} attended</Text>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['left', 'right']}>
      <FlatList
        ...
        ListHeaderComponent={
          <Text style={styles.heading}>📅 My Subjects ({subjects.length})</Text>
        }
        ListEmptyComponent={
...
const styles = StyleSheet.create({
  ...
  numBadge: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#eef2ff', alignItems: 'center', justifyContent: 'center' },
  ...
  bar: { height: 6, borderRadius: 3, backgroundColor: '#e2e8f0', marginTop: 12, overflow: 'hidden' },
