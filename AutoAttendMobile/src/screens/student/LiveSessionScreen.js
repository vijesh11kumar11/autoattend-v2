...
export default function LiveSessionScreen({ route }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[LiveSession] fetch error:', err?.message);
    }
  }, [session_id]);
      ...
      <View style={[styles.statusCard, { backgroundColor: isLive ? '#dcfce7' : '#fee2e2' }]}>
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.statusSub} numberOfLines={1}>
            {data.subject_name ?? data.subject ?? data.title ?? ''}
          </Text>
        </View>
      </View>
    ...
    >
      ...
      <View style={styles.infoCard}>
        <InfoRow
          icon="person-outline"
          label="Teacher"
          value={data.teacher_name ?? data.host_name ?? '—'}
        />
        <InfoRow
          icon="time-outline"
          label="Started"
          value={(data.started_at ?? '').replace('T', ' ').slice(0, 16) || '—'}
        />
        <InfoRow
          icon="people-outline"
          label="Joined"
          value={data.participant_count ?? data.participants?.length ?? 0}
        />
...
function DoubtsTab({ session_id, isLive }) {
  ...
  const fetchList = useCallback(async () => {
    try {
      ...
      console.warn('[Doubts] fetch error:', err?.message);
    }
  }, [session_id]);
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const { data } = await client.post(`/live/sessions/${session_id}/doubts`, {
        question_text: t,
      });
      setText('');
      if (data?.ai_suggestion) {
        Alert.alert(
          'Doubt Posted',
          `AI suggestion (confidence ${(data.confidence * 100) | 0}%):\n\n${data.ai_suggestion}`
        );
      } else {
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.composer}>
          <TextInput
            ...
            placeholder="Posted anonymously to the class wall…"
            placeholderTextColor="#94a3b8"
            multiline
            maxLength={1000}
          />
          <TouchableOpacity
            style={[styles.composerBtn, (busy || !isLive) && { opacity: 0.5 }]}
            onPress={submit}
            ...
            <View key={d.id} style={styles.doubt}>
              ...
              {d.teacher_answer ? (
                <View style={styles.ans}>
                  <Text style={styles.ansLabel}>Teacher:</Text>
                  <Text style={styles.ansTxt}>{d.teacher_answer}</Text>
                </View>
              ) : d.ai_suggested_answer ? (
                <View style={styles.aiAns}>
                  <Text style={styles.ansLabel}>AI ({((d.ai_confidence ?? 0) * 100) | 0}%):</Text>
                  <Text style={styles.ansTxt}>{d.ai_suggested_answer}</Text>
                </View>
              ) : null}
            </View>
          ))
...
function PulseTab({ session_id, pulses }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.scroll}>
      ...
      {open.length === 0 ? (
        <Text style={styles.empty}>No pulse checks right now.</Text>
      ) : (
        open.map((p) => {
              ...
              {['A', 'B', 'C', 'D'].map((k) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.optBtn,
                      sel && styles.optBtnActive,
                      busy === p.id && { opacity: 0.6 },
                    ]}
                    onPress={() => !my && respond(p.id, k)}
                    disabled={!!my || busy === p.id}
                  >
    ...
    return (
      <View style={styles.empty2}>
        ...
        <Text style={styles.emptySub}>
          Your teacher hasn't generated personalised questions for you.
        </Text>
      </View>
    );
  }
...
function LowBwTab({ session_id }) {
  ...
  const fetchSummary = useCallback(async () => {
    try {
      ...
      console.warn('[LowBW] error:', err?.message);
    }
  }, [session_id]);
  ...
  return (
    ...
    >
      ...
      <Text style={styles.sub}>
        Text-only AI summary of the live session — friendly to slow networks.
      </Text>
      <View style={styles.summaryCard}>
        <Text style={styles.summaryTxt}>
          {text || 'No summary yet — try refreshing in a few seconds.'}
        </Text>
      </View>
    </ScrollView>
  );
