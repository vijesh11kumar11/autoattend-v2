...
export default function MyTutorScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch (err) { console.warn('[MyTutor] fetch error:', err?.message); setTutor(null); }
  }, []);
          ...
          <>
            <View style={styles.card}>
              <View style={styles.avatar}>
                <Text style={styles.avatarTxt}>{(tutor.name ?? tutor.tutor_name ?? '?').charAt(0).toUpperCase()}</Text>
              </View>
            ...
            {tutor.phone && (
              <TouchableOpacity style={styles.actionRow} onPress={() => Linking.openURL(`tel:${tutor.phone}`)}>
            ...
            {tutor.email && (
              <TouchableOpacity style={styles.actionRow} onPress={() => Linking.openURL(`mailto:${tutor.email}`)}>
            ...
            {tutor.whatsapp && (
              <TouchableOpacity style={styles.actionRow} onPress={() => Linking.openURL(`https://wa.me/${tutor.whatsapp.replace(/\D/g, '')}`)}>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 14, padding: 20, alignItems: 'center', borderWidth: 1, borderColor: '#e2e8f0', marginBottom: 14 },
  avatar: { width: 72, height: 72, borderRadius: 36, backgroundColor: PRIMARY, alignItems: 'center', justifyContent: 'center', marginBottom: 12 },
  ...
  actionRow: { backgroundColor: '#fff', borderRadius: 12, padding: 14, marginBottom: 8, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#e2e8f0' },
  actLabel: { fontSize: 11, color: '#94a3b8', fontWeight: '700' },
  actValue: { fontSize: 14, color: '#1e293b', fontWeight: '600', marginTop: 2 },
  emptyCard: { backgroundColor: '#fff', borderRadius: 14, padding: 30, alignItems: 'center', borderWidth: 1, borderColor: '#e2e8f0' },
