...
export default function MySessionsScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch (err) { console.warn('[MySessions] fetch error:', err?.message); }
  }, []);
      ...
      <ScrollView contentContainerStyle={styles.scroll}
        ...
        {live.length === 0
          ...
          : live.slice(0, 20).map((s, i) => {
              ...
              return (
                <TouchableOpacity key={s.id ?? i} style={styles.row}
                  onPress={() => navigation.navigate('LiveSession', { session_id: s.id })}>
                  <Ionicons name={active ? 'radio' : 'radio-outline'} size={20} color={active ? '#ef4444' : '#94a3b8'} />
                  <View style={{ flex: 1, marginLeft: 10 }}>
                    <Text style={styles.title}>{s.subject_name ?? s.subject ?? 'Session'}</Text>
                    <Text style={styles.meta}>
                      {active ? 'LIVE NOW' : (s.ended_at ?? s.end_time ?? s.created_at ?? '').slice(0, 16).replace('T', ' ')}
                    </Text>
                  </View>
          ...
          : today.map((c, i) => (
              <View key={c.id ?? i} style={styles.row}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  <Text style={styles.meta}>
                    {c.start_time ?? c.from_time ?? '—'}{c.end_time || c.to_time ? ` – ${c.end_time ?? c.to_time}` : ''}
                    {c.faculty_name ? ` · ${c.faculty_name}` : ''}
                  </Text>
                </View>
                {c.status && (
                  <View style={[styles.badge, c.status === 'active' && { backgroundColor: '#dcfce7' }]}>
                    <Text style={[styles.badgeTxt, c.status === 'active' && { color: '#15803d' }]}>{c.status}</Text>
                  </View>
                )}
              </View>
            ))}
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 12, borderRadius: 10, marginBottom: 6, borderWidth: 1, borderColor: '#e2e8f0' },
