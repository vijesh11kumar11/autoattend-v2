...
export default function StudentKnowledgeGraphScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setGraph(data);
    } catch (err) { console.warn('[KG] fetch error:', err?.message); }
  }, []);
      ...
      <ScrollView contentContainerStyle={styles.scroll}
        ...
        {overall != null && (
          <View style={styles.overall}>
            ...
            <Text style={[styles.overallVal, { color: masteryColor(overall) }]}>{Number(overall).toFixed(0)}%</Text>
          </View>
        )}
        ...
        {!subjects.length ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySub}>Attend live sessions and answer pulse-checks to build your graph.</Text>
          </View>
        ) : (
          subjects.map((s, i) => {
            ...
            return (
              <View key={key} style={styles.subjCard}>
                <TouchableOpacity style={styles.subjHead} onPress={() => setExpanded(p => ({ ...p, [key]: !p[key] }))}>
                  <View style={[styles.dot, { backgroundColor: masteryColor(mastery) }]} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.subjName}>{s.name ?? s.subject_name ?? 'Subject'}</Text>
                    <Text style={styles.subjMeta}>{topics.length} topic{topics.length === 1 ? '' : 's'}</Text>
                  </View>
                  <Text style={[styles.subjPct, { color: masteryColor(mastery) }]}>{Number(mastery).toFixed(0)}%</Text>
                  ...
                  return (
                    <View key={j} style={styles.topicRow}>
                      ...
                      <View style={styles.barTrack}>
                        <View style={[styles.barFill, { width: `${Math.min(100, tm)}%`, backgroundColor: masteryColor(tm) }]} />
                      </View>
                      <Text style={[styles.topicPct, { color: masteryColor(tm) }]}>{Number(tm).toFixed(0)}%</Text>
                    </View>
                  );
                })}
...
const styles = StyleSheet.create({
  ...
  overall: { backgroundColor: '#fff', borderRadius: 12, padding: 14, alignItems: 'center', marginBottom: 14, borderWidth: 1, borderColor: '#e2e8f0' },
  overallLabel: { fontSize: 11, color: '#94a3b8', fontWeight: '700' },
  overallVal: { fontSize: 28, fontWeight: '900', marginTop: 4 },
  subjCard: { backgroundColor: '#fff', borderRadius: 12, marginBottom: 10, borderWidth: 1, borderColor: '#e2e8f0', overflow: 'hidden' },
  ...
  topicRow: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 10, paddingLeft: 26, borderTopWidth: 1, borderTopColor: '#f1f5f9' },
  topicDot: { width: 6, height: 6, borderRadius: 3 },
  topicName: { flex: 1, fontSize: 12, color: '#475569', fontWeight: '600' },
  barTrack: { width: 70, height: 6, backgroundColor: '#f1f5f9', borderRadius: 3, overflow: 'hidden' },
  barFill: { height: '100%', borderRadius: 3 },
  topicPct: { fontSize: 11, fontWeight: '700', minWidth: 36, textAlign: 'right' },
  empty: { alignItems: 'center', paddingTop: 40, backgroundColor: '#fff', borderRadius: 12, padding: 24, borderWidth: 1, borderColor: '#e2e8f0' },
