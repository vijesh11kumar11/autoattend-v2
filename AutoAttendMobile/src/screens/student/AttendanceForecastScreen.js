...
export default function AttendanceForecastScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('[Forecast] fetch error:', err?.message);
    }
  }, []);
      ...
      <FlatList
            ...
            {overall && (
              <View style={styles.overallCard}>
                <Ionicons
                  ...
                  size={22}
                  color={pctColor(overall.percentage ?? 0)}
                />
                <View style={{ flex: 1, marginLeft: 10 }}>
                  <Text style={styles.overallLabel}>Overall</Text>
                  <Text style={[styles.overallVal, { color: pctColor(overall.percentage ?? 0) }]}>
                    {(overall.percentage ?? 0).toFixed(1)}% · {overall.present ?? 0}/
                    {overall.total ?? 0}
                  </Text>
                </View>
              </View>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyTxt}>No forecast data.</Text>
          </View>
        }
        renderItem={({ item: s }) => {
          ...
          return (
            <View style={[styles.card, { borderLeftColor: pctColor(pct) }]}>
              <View style={styles.cardHead}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.subjMeta}>
                    {s.code ?? s.subject_code ?? ''} · {s.present ?? 0}/{s.total ?? 0}
                  </Text>
                </View>
              ...
              <View style={[styles.advice, { backgroundColor: safe ? '#f0fdf4' : '#fef2f2' }]}>
                <Ionicons
                  name={safe ? 'checkmark-circle' : 'warning'}
                  size={18}
