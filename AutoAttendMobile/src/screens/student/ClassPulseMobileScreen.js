...
export function ClassPulseHomeScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      console.warn('[ClassPulseMobileScreen] fetch error:', err?.message);
    }
  }, []);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <SubjectCard
            item={item}
            onPress={() =>
              navigation.navigate('ClassPulseSubject', {
...
export function ClassPulseSubjectScreen({ route, navigation }) {
  ...
  useEffect(() => {
    navigation.setOptions({ title: subjectName || 'Subject' });
  ...
  return (
    <SafeAreaView style={s.root} edges={['left', 'right']}>
      <View style={s.subjectHeader}>
        ...
        {attendancePct !== undefined && (
          <View
            style={[
              ...
              {
                backgroundColor:
                  attendancePct >= 75 ? '#dcfce7' : attendancePct >= 60 ? '#fef3c7' : '#fee2e2',
              },
            ]}
          >
            <Text
              style={[
                ...
                {
                  color:
                    attendancePct >= 75 ? '#15803d' : attendancePct >= 60 ? '#b45309' : '#b91c1c',
                },
              ]}
            >
              {attendancePct}%
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate('ClassWall', { subjectId, subjectName })}
          style={s.wallBtn}
        >
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setDenyInfo(null)}
      >
    ...
    <TouchableOpacity style={s.capsuleCard} onPress={onPress}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        <Text style={{ fontSize: 18 }}>
          {c.capsule_type === 'voice_note'
            ? '🎙️'
            : c.capsule_type === 'video'
              ? '🎥'
              : c.capsule_type === 'image'
                ? '🖼️'
                : '📄'}
        </Text>
        <Text style={[s.capsuleTitle, { flex: 1 }]} numberOfLines={2}>
          {c.title}
        </Text>
      </View>
      {banner && (
        <View
          style={[
            s.accessBanner,
            { borderColor: banner.tone, backgroundColor: banner.tone + '15' },
          ]}
        >
        ...
        <View style={s.interactionRow}>
          ...
          {c.my_interaction.quiz_attempted && (
            <Tag
              ...
              text={
                c.my_interaction.quiz_passed
                  ? `Quiz ✓ ${c.my_interaction.quiz_score}`
                  : `Quiz ✗ ${c.my_interaction.quiz_score}`
              }
            />
          )}
...
export function CapsuleMobileDetailScreen({ route, navigation }) {
  ...
  useEffect(() => {
    navigation.setOptions({ title: capsuleTitle || 'Capsule' });
  ...
  useEffect(
    () => () => {
      if (audioState.sound) audioState.sound.unloadAsync().catch(() => {});
    },
    [audioState.sound]
  );
  ...
  const openDocument = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        'Please install expo-file-system + expo-web-browser, or check your connection.'
      );
    }
  };
  ...
  if (!data)
    return (
      <Center>
        <Text>Failed to load capsule.</Text>
      </Center>
    );
      ...
      <ScrollView contentContainerStyle={{ padding: 12, paddingBottom: 60 }}>
            ...
            <TouchableOpacity onPress={playVoice} style={s.playBtn} disabled={audioState.loading}>
              {audioState.loading ? (
                ...
                <Text style={s.playBtnText}>
                  {audioState.playing ? '⏸ Pause' : '▶ Play voice memo'}
                </Text>
              )}
            </TouchableOpacity>
          </View>
        ...
        {hasFile && (
          <View style={s.section}>
            ...
            {!hasOpenedDoc && (
              <Text style={s.muted}>Open the document to unlock the comprehension quiz.</Text>
            )}
          </View>
        )}
        ...
        {hasOpenedDoc && quiz && Array.isArray(quiz.questions) && quiz.questions.length > 0 && (
          <QuizSection
            capsuleId={capsuleId}
            quiz={quiz}
            alreadyAttempted={data.my_interaction?.quiz_attempted}
            previousScore={data.my_interaction?.quiz_score}
            previousPassed={data.my_interaction?.quiz_passed}
          />
        )}
        ...
        <View style={s.section}>
          ...
          <TouchableOpacity
            onPress={submitDoubt}
            disabled={postingDoubt || !doubt.trim()}
            style={[s.postBtn, (postingDoubt || !doubt.trim()) && { opacity: 0.5 }]}
          >
...
function QuizSection({ capsuleId, quiz, alreadyAttempted, previousScore, previousPassed }) {
  ...
  const [result, setResult] = useState(
    alreadyAttempted ? { score: previousScore, passed: previousPassed, prior: true } : null
  );
  ...
  if (result) {
    return (
      ...
      >
        ...
        <Text style={[s.bigScore, { color: result.passed ? '#15803d' : '#b91c1c' }]}>
          {result.score}%
        </Text>
        <Text
          style={[s.bullet, { fontWeight: '700', color: result.passed ? '#15803d' : '#b91c1c' }]}
        >
...
export function ClassWallMobileScreen({ route, navigation }) {
  ...
  useEffect(() => {
    navigation.setOptions({ title: `💬 ${subjectName || 'Wall'}` });
  }, [navigation, subjectName]);
    ...
    setPosts((p) =>
      p.map((x) =>
        x.id === post.id
          ? {
              ...x,
              resonance_count: (x.resonance_count || 0) + (x.has_resonated ? -1 : 1),
              has_resonated: !x.has_resonated,
            }
          : x
      )
    );
        ...
        >
          <Pressable style={s.sheetBackdrop} onPress={() => setShowAdd(false)}>
            <Pressable style={s.sheet} onPress={(e) => e.stopPropagation()}>
              ...
              />
              <TouchableOpacity
                style={[s.sheetBtn, (!newDoubt.trim() || posting) && { opacity: 0.5 }]}
                onPress={submit}
...
function WallPost({ p, onResonate }) {
  ...
  return (
    <View style={s.post}>
      ...
      {p.related_capsule_title && <Text style={s.postLink}>📎 {p.related_capsule_title}</Text>}
      <View style={{ flexDirection: 'row', gap: 12, marginTop: 8 }}>
        <TouchableOpacity
          onPress={onResonate}
          style={[s.resonateBtn, p.has_resonated && { backgroundColor: '#ddd6fe' }]}
        >
          <Text style={[s.resonateText, p.has_resonated && { color: VIOLET, fontWeight: '700' }]}>
            🤝 {p.resonance_count || 0}
          </Text>
        </TouchableOpacity>
        <View
          style={[
            s.statusPill,
            { backgroundColor: p.status === 'answered' ? '#dcfce7' : '#fef3c7' },
          ]}
        >
          <Text
            style={[s.statusPillText, { color: p.status === 'answered' ? '#166534' : '#b45309' }]}
          >
            {p.status}
          </Text>
        </View>
