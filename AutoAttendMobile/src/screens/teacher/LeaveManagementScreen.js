...
export default function LeaveManagementScreen() {
  ...
  useEffect(() => { fetchPending().finally(() => setLoading(false)); }, [fetchPending]);
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View style={styles.card}>
                ...
                <Text style={styles.reasonText} numberOfLines={4}>{item.reason}</Text>

                {item.document_url ? (
                  <View style={styles.docRow}>
                    <Ionicons name="attach" size={14} color="#3b82f6" />
                    <Text style={styles.docText} numberOfLines={1}>{item.document_url}</Text>
                  </View>
                ) : null}
          ...
          <View style={styles.modalCard}>
            ...
            {modalLeave?.leave ? (
              <Text style={styles.modalSub}>
                {modalLeave.leave.student_name} · {modalLeave.leave.from_date} → {modalLeave.leave.to_date}
              </Text>
            ) : null}
            ...
            <TextInput
              ...
              placeholder={modalLeave?.action === 'reject' ? 'Why is this being rejected?' : 'Optional comment'}
              placeholderTextColor="#94a3b8"
              multiline
            />

            <View style={styles.modalActionRow}>
              <TouchableOpacity style={styles.modalCancel} onPress={closeModal} disabled={submitting}>
...
const styles = StyleSheet.create({
  ...
  modalCard:     { backgroundColor: '#fff', borderTopLeftRadius: 18, borderTopRightRadius: 18, padding: 20 },
  modalTitle:    { fontSize: 18, fontWeight: '700', color: '#1e293b' },
  modalSub:      { fontSize: 12, color: '#64748b', marginTop: 4 },
  label:         { fontSize: 12, fontWeight: '700', color: '#64748b', textTransform: 'uppercase', marginTop: 14, marginBottom: 6, letterSpacing: 0.4 },
  input: {
  ...
  modalCancel:    { flex: 1, paddingVertical: 12, borderRadius: 10, borderWidth: 1, borderColor: '#cbd5e1', alignItems: 'center' },
