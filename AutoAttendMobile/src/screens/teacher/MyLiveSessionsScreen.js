...
export default function MyLiveSessionsScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    } catch (err) { console.warn('[MyLive] error:', err?.message); }
  }, []);
          ...
          return (
            <View key={s.id ?? i} style={styles.card}>
              <View style={styles.cardHead}>
                <Ionicons name={live ? 'radio' : 'radio-outline'} size={18} color={live ? '#ef4444' : '#94a3b8'} />
                <Text style={styles.cardTitle}>{s.subject_name ?? s.title ?? 'Session'}</Text>
                {live && <View style={styles.livePill}><Text style={styles.livePillTxt}>LIVE</Text></View>}
              </View>
              ...
              <View style={styles.btnRow}>
                {live && (
                  <TouchableOpacity style={[styles.btn, { backgroundColor: PRIMARY }]}
                    onPress={() => navigation.navigate('LiveSessionDashboard', { session_id: s.id })}>
                ...
                {!live && (
                  <TouchableOpacity style={[styles.btn, { backgroundColor: '#22c55e' }]}
                    onPress={() => navigation.navigate('SessionHealthReport', { session_id: s.id })}>
...
const styles = StyleSheet.create({
  ...
  livePill: { backgroundColor: '#ef4444', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
