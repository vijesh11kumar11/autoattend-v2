...
export default function SessionHealthReportScreen({ route }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[Health] error:', err?.message);
    }
  }, [session_id]);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        {data.subject_name ? (
          <InfoRow icon="book-outline" label="Subject" value={data.subject_name} />
        ) : null}
        {data.attendance_percentage != null ? (
          <InfoRow
            icon="people-outline"
            label="Attendance"
            value={`${data.attendance_percentage.toFixed(1)}%`}
          />
        ) : null}
        {data.duration_minutes != null ? (
          <InfoRow icon="time-outline" label="Duration" value={`${data.duration_minutes} min`} />
        ) : null}
