...
export default function PreClassBriefScreen({ route }) {
  ...
  const fetchData = useCallback(async () => {
    if (!session_id) { setError('Missing session_id'); return; }
    try {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll}
        ...
        {error && <View style={styles.errBox}><Ionicons name="warning-outline" size={18} color="#b91c1c" /><Text style={styles.errTxt}>{error}</Text></View>}

        {data && (
          <>
            <Section title="📌 Summary" body={data.summary ?? data.overview ?? null} />
            <Section title="🔄 Last Session Recap" body={data.last_session_recap ?? data.previous_summary ?? null} />
            <ListSection title="⚠️ Weak Topics" items={data.weak_topics ?? data.gaps ?? []} icon="alert-circle-outline" color="#ef4444" />
            <ListSection title="💡 Suggested Talking Points" items={data.suggested_topics ?? data.talking_points ?? data.suggestions ?? []} icon="bulb-outline" color="#f59e0b" />
            <ListSection title="🎯 Pre-requisites" items={data.prerequisites ?? []} icon="checkmark-circle-outline" color="#22c55e" />
            <ListSection title="📖 Key Concepts" items={data.key_concepts ?? data.concepts ?? []} icon="bookmark-outline" color={PRIMARY} />

            {!data.summary && !(data.weak_topics?.length) && !(data.suggested_topics?.length) && (
              <View style={styles.rawCard}>
                <Text style={{ fontSize: 11, color: '#64748b' }} selectable>{JSON.stringify(data, null, 2)}</Text>
              </View>
            )}
          </>
        )}
    ...
    <View style={styles.card}>
      ...
      {items.map((it, i) => (
        <View key={i} style={styles.bullet}>
          ...
          <Text style={styles.bulletTxt}>{typeof it === 'string' ? it : (it.title ?? it.name ?? it.text ?? JSON.stringify(it))}</Text>
        </View>
      ))}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  errBox: { flexDirection: 'row', backgroundColor: '#fef2f2', padding: 12, borderRadius: 10, gap: 8, alignItems: 'center', marginBottom: 14 },
  errTxt: { flex: 1, color: '#b91c1c', fontSize: 12, fontWeight: '600' },
  rawCard: { backgroundColor: '#fff', borderRadius: 10, padding: 10, borderWidth: 1, borderColor: '#e2e8f0' },
});
