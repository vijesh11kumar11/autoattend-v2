...
export function TeacherClassPulseHomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    client.get('/classpulse/teacher/dashboard')
      .then(r => {
        if (cancelled) return;
        const list = (r.data?.subjects || []).map(x => ({ id: x.subject_id, name: x.subject_name }));
        setSubjects(list);
  ...
  if (loadingSubjects) return <Center><ActivityIndicator color={PRIMARY} /></Center>;
  if (subjects.length === 0) return <Center><Text style={s.muted}>No subjects assigned to you.</Text></Center>;
  ...
  return (
    <SafeAreaView style={s.root} edges={['left', 'right']}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ padding: 8 }}>
        {subjects.map(sb => (
          <TouchableOpacity key={sb.id} onPress={() => setActiveSubject(sb)}
            style={[s.chip, activeSubject?.id === sb.id && s.chipActive]}>
            <Text style={[s.chipText, activeSubject?.id === sb.id && s.chipTextActive]}>{sb.name}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
...
function CapsulesTab({ subject, navigation }) {
  ...
  if (!subject) return <Center><Text style={s.muted}>Select a subject.</Text></Center>;
  ...
  return (
    <FlatList
      ...
      refreshControl={<RefreshControl refreshing={refreshing} onRefresh={async () => { setRefreshing(true); await load(); setRefreshing(false); }} colors={[PRIMARY]} />}
      ListEmptyComponent={<Empty text="No capsules in this subject." />}
      renderItem={({ item }) => (
        <TouchableOpacity style={s.card} onPress={() => navigation.navigate('CapsuleAnalytics', { capsuleId: item.id, capsuleTitle: item.title })}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <Text style={{ fontSize: 18 }}>{iconFor(item.capsule_type)}</Text>
            <Text style={[s.cardTitle, { flex: 1 }]} numberOfLines={2}>{item.title}</Text>
          </View>
          <View style={s.statRow}>
            <Stat label="Views"     value={item.view_count ?? 0} />
            <Stat label="Downloads" value={item.download_count ?? 0} />
            <Stat label="Quiz Pass" value={`${item.quiz_pass_pct ?? 0}%`} tone={item.quiz_pass_pct >= 70 ? '#15803d' : item.quiz_pass_pct >= 50 ? '#b45309' : '#b91c1c'} />
          </View>
        </TouchableOpacity>
      )}
    />
...
function WallTab({ subject, navigation }) {
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => (
        <TouchableOpacity style={s.card} onPress={() => navigation.navigate('AnswerDoubt', { post: item })}>
          ...
          <View style={s.postRow}>
            ...
            <View style={[s.statusPill, { backgroundColor: item.status === 'answered' ? '#dcfce7' : '#fef3c7' }]}>
              <Text style={[s.statusPillText, { color: item.status === 'answered' ? '#166534' : '#b45309' }]}>{item.status}</Text>
            </View>
          </View>
        </TouchableOpacity>
      )}
...
export function CapsuleAnalyticsMobileScreen({ route, navigation }) {
  ...
  useEffect(() => { navigation.setOptions({ title: capsuleTitle || 'Analytics' }); }, [navigation, capsuleTitle]);

  useEffect(() => {
    let cancelled = false;
    client.get(`/classpulse/teacher/capsule/${capsuleId}/analytics`)
  ...
  if (loading) return <Center><ActivityIndicator color={PRIMARY} /></Center>;
  if (!data)   return <Center><Text style={s.muted}>Failed to load analytics.</Text></Center>;
      ...
      <ScrollView contentContainerStyle={{ padding: 12 }}>
        <View style={s.section}>
          ...
          <View style={s.statRow}>
            ...
            <Stat label="Quiz Pass %" value={`${data.quiz_pass_pct ?? 0}%`} tone={data.quiz_pass_pct >= 70 ? '#15803d' : '#b45309'} />
            <Stat label="Downloads" value={data.download_count ?? 0} />
          </View>
        </View>

        {atRisk.length > 0 && (
          <View style={[s.section, { backgroundColor: '#fef2f2', borderColor: '#fecaca', borderWidth: 1 }]}>
          ...
          {students.length === 0 ? (
            ...
            <View key={st.student_id} style={s.studentRow}>
              ...
              <View style={{ alignItems: 'flex-end' }}>
                <Text style={[s.tag, { color: st.opened ? '#15803d' : '#94a3b8' }]}>{st.opened ? '📖 Opened' : '⬜ Not opened'}</Text>
                {st.quiz_attempted && (
                  <Text style={[s.tag, { color: st.quiz_passed ? '#15803d' : '#b91c1c' }]}>
                    {st.quiz_passed ? '✅' : '❌'} {st.quiz_score}%
                  </Text>
                )}
                {st.downloaded && <Text style={[s.tag, { color: '#3b82f6' }]}>⬇️ Downloaded</Text>}
              </View>
            </View>
          ))}
        </View>
...
export function AnswerDoubtMobileScreen({ route, navigation }) {
  ...
  useEffect(() => { navigation.setOptions({ title: '💬 Answer Doubt' }); }, [navigation]);
  ...
  if (!post) return <Center><Text style={s.muted}>Missing post.</Text></Center>;
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: 12 }}>
          <View style={s.section}>
            ...
            {post.is_hot && <Text style={[s.hotBadge, { marginTop: 6 }]}>🔥 Hot — {post.resonance_count} students resonated</Text>}
          </View>
...
// Helpers
// ═══════════════════════════════════════════════════════════════════════
const iconFor = (t) => t === 'voice_note' ? '🎙️' : t === 'video' ? '🎥' : t === 'image' ? '🖼️' : '📄';
...
const s = StyleSheet.create({
  ...
  chip: { backgroundColor: '#fff', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999, marginRight: 6, borderWidth: 1, borderColor: '#e2e8f0' },
  ...
  statLabel: { fontSize: 10, color: '#64748b', textTransform: 'uppercase', fontWeight: '700', marginTop: 2 },
  hotBadge: { fontSize: 11, fontWeight: '700', color: '#ea580c', marginBottom: 6 },
  postContent: { fontSize: 14, color: '#0f172a', marginTop: 4 },
  postRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 },
  ...
  studentRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderColor: '#f1f5f9' },
  studentName: { fontSize: 13, fontWeight: '700', color: '#0f172a' },
  tag: { fontSize: 11, fontWeight: '700', marginTop: 2 },
  textarea: { borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 8, padding: 10, minHeight: 100, textAlignVertical: 'top', backgroundColor: '#f8fafc', color: '#0f172a' },
  postBtn: { backgroundColor: VIOLET, paddingVertical: 12, borderRadius: 10, alignItems: 'center', marginTop: 10 },
