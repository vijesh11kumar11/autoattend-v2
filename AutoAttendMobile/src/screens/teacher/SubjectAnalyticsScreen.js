...
export default function SubjectAnalyticsScreen({ route }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[SubjectAnalytics] fetch error:', err?.message);
    }
  }, [subject_id]);
  ...
  if (!data)
    return (
      <View style={styles.center}>
        <Text style={styles.errTxt}>Failed to load analytics.</Text>
      </View>
    );
      ...
      >
        ...
        <View style={styles.statsRow}>
          <View style={styles.stat}>
            ...
            <Text style={styles.statL}>Students</Text>
          </View>
          <View style={styles.stat}>
            <Ionicons name="calendar-outline" size={20} color={PRIMARY} />
            <Text style={styles.statV}>{sessions}</Text>
