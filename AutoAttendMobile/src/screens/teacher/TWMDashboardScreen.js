...
export default function TWMDashboardScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (h.status === 'fulfilled') setHistory(Array.isArray(h.value.data) ? h.value.data : h.value.data?.sessions ?? []);
    } catch (err) { console.warn('[TWM] fetch error:', err?.message); }
  }, []);
  ...
  const markStudent = async (student_id, status) => {
    ...
    try {
      ...
      setActiveSession(prev => ({
        ...prev,
        ward_students: prev.ward_students.map(s => s.student_id === student_id ? { ...s, status } : s),
      }));
    } catch (err) { Alert.alert('Error', 'Mark failed.'); }
  };
    ...
    Alert.alert('End TWM', 'Submit and end this session?', [
      ...
      { text: 'End', style: 'destructive', onPress: async () => {
        try {
          ...
          setTab('Wards');
          fetchData();
        } catch (err) { Alert.alert('Error', 'Could not end.'); }
      }},
    ]);
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      {tab === 'Wards' && (
        <ScrollView contentContainerStyle={styles.scroll}
          ...
          <Text style={styles.sub}>{data?.academic_year ? `Academic Year ${data.academic_year}` : 'Tutor-with-Mentor program'}</Text>
          ...
          <View style={styles.statsRow}>
            ...
            <Stat label="Critical" value={(summary.critical ?? 0) + (summary.detained ?? 0)} color="#ef4444" />
          </View>

          {!activeSession && (
            <TouchableOpacity style={[styles.startBtn, starting && { opacity: 0.6 }]} onPress={startTWM} disabled={starting}>
              {starting ? <ActivityIndicator color="#fff" /> :
                <><Ionicons name="play-circle-outline" size={18} color="#fff" /><Text style={styles.startBtnTxt}>Start TWM Session (Today)</Text></>}
            </TouchableOpacity>
          )}

          <Text style={styles.section}>Wards ({wards.length})</Text>
          {wards.length === 0 ? <Text style={styles.empty}>No ward students assigned.</Text> :
            wards.map(s => (
              <View key={s.student_id} style={[styles.wardRow, { borderLeftColor: stColor(s.attendance_status === 'safe' ? 'present' : s.attendance_status === 'warning' ? 'late' : 'absent') }]}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.wardName}>{s.name}</Text>
                  <Text style={styles.wardMeta}>{s.roll_number ?? ''}{s.section ? ` · ${s.section}` : ''}</Text>
                </View>
                <View>
                  <Text style={[styles.wardPct, { color: stColor(s.attendance_status === 'safe' ? 'present' : s.attendance_status === 'warning' ? 'late' : 'absent') }]}>
        ...
        <ScrollView contentContainerStyle={styles.scroll}>
          {!activeSession ? (
            ...
            <>
              ...
              <Text style={styles.sub}>{activeSession.date} · {activeSession.total ?? activeSession.ward_students?.length ?? 0} students</Text>

              <View style={{ flexDirection: 'row', gap: 8 }}>
                <TouchableOpacity style={[styles.bulkBtn, { backgroundColor: '#22c55e' }]} onPress={markAllPresent}>
                  <Ionicons name="checkmark-done-outline" size={16} color="#fff" />
                  <Text style={styles.bulkBtnTxt}>All Present</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.bulkBtn, { backgroundColor: '#ef4444' }]} onPress={endSession}>
                  ...
                  <View style={styles.markBtns}>
                    {ATT_STATES.map(st => (
                      <TouchableOpacity key={st}
                        style={[styles.markBtn, s.status === st && { backgroundColor: stColor(st), borderColor: stColor(st) }]}
      ...
      {tab === 'History' && (
        <ScrollView contentContainerStyle={styles.scroll}
          ...
          {history.length === 0 ? <Text style={styles.empty}>No past sessions.</Text> :
            history.map(h => (
              <View key={h.id ?? h.session_id} style={styles.histRow}>
                <Ionicons name="calendar-outline" size={20} color={PRIMARY} />
                <View style={{ flex: 1, marginLeft: 10 }}>
                  <Text style={styles.wardName}>{h.date}</Text>
                  <Text style={styles.wardMeta}>{h.present_count ?? 0}/{h.total ?? 0} present · {h.status ?? 'ended'}</Text>
                </View>
              </View>
            ))}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  stat: { flex: 1, backgroundColor: '#fff', borderRadius: 10, padding: 12, alignItems: 'center', borderWidth: 1, borderColor: '#e2e8f0' },
  statV: { fontSize: 22, fontWeight: '900' },
  statL: { fontSize: 10, color: '#94a3b8', fontWeight: '700', marginTop: 2 },
  startBtn: { flexDirection: 'row', backgroundColor: PRIMARY, padding: 14, borderRadius: 12, alignItems: 'center', justifyContent: 'center', gap: 8 },
  startBtnTxt: { color: '#fff', fontWeight: '700', fontSize: 14 },
  bulkBtn: { flex: 1, flexDirection: 'row', padding: 12, borderRadius: 10, alignItems: 'center', justifyContent: 'center', gap: 6 },
  bulkBtnTxt: { color: '#fff', fontWeight: '700', fontSize: 12 },
  wardRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 12, borderRadius: 10, marginBottom: 6, borderWidth: 1, borderColor: '#e2e8f0', borderLeftWidth: 4 },
  ...
  markRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 10, borderRadius: 10, marginBottom: 6, borderWidth: 1, borderColor: '#e2e8f0' },
  markBtns: { flexDirection: 'row', gap: 4 },
  markBtn: { width: 32, height: 32, borderRadius: 6, backgroundColor: '#f8fafc', borderWidth: 1, borderColor: '#e2e8f0', alignItems: 'center', justifyContent: 'center' },
  markBtnTxt: { fontSize: 12, fontWeight: '800', color: '#475569' },
  histRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 12, borderRadius: 10, marginBottom: 6, borderWidth: 1, borderColor: '#e2e8f0' },
