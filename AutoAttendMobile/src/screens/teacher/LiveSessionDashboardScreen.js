    ...
    Alert.alert('End Live Session', 'End this session? This cannot be undone.', [
      ...
      { text: 'End', style: 'destructive', onPress: async () => {
        ...
        try {
          ...
          Alert.alert('Ended', 'Session ended.', [{ text: 'OK', onPress: () => navigation.goBack() }]);
        } catch (err) {
    ...
    <SafeAreaView style={styles.safe}>
      <View style={[styles.statusCard, { backgroundColor: isLive ? '#dcfce7' : '#fee2e2' }]}>
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          <Text style={[styles.statusTxt, { color: isLive ? '#15803d' : '#b91c1c' }]}>{isLive ? '🔴 LIVE' : 'Ended'}</Text>
          <Text style={styles.statusSub} numberOfLines={1}>{data.subject_name ?? data.title ?? '—'}</Text>
        </View>
        {isLive && (
          <TouchableOpacity style={styles.endBtn} onPress={endSession} disabled={ending}>
            {ending ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.endTxt}>End</Text>}
          </TouchableOpacity>
        )}
      </View>
      ...
      {tab === 'Overview' && <OverviewTab data={data} refreshing={refreshing} onRefresh={onRefresh} session_id={session_id} navigation={navigation} />}
      {tab === 'Doubts'   && <DoubtsTab session_id={session_id} />}
      {tab === 'Pulse'    && <PulseTab session_id={session_id} pulses={data.pulse_checks ?? []} isLive={isLive} />}
      {tab === 'Brain'    && <BrainTab session_id={session_id} isLive={isLive} />}
      {tab === 'Breakout' && <BreakoutTab session_id={session_id} isLive={isLive} participants={data.participants ?? []} />}
    </SafeAreaView>
  );
}
  ...
  return (
    <ScrollView contentContainerStyle={styles.scroll}
      ...
      <View style={styles.statsRow}>
        ...
        <Stat icon="pulse-outline" v={data.pulse_count ?? data.pulse_checks?.length ?? 0} l="Pulses" />
      </View>
...
function DoubtsTab({ session_id }) {
  ...
  const fetchList = useCallback(async () => {
    ...
    } catch (err) { console.warn('[Doubts] error:', err?.message); }
  }, [session_id]);
  ...
  const submitAnswer = async () => {
    ...
    try {
      await client.post(`/live/sessions/${session_id}/ai/teacher-response`, {
        post_id: answering.id, response: answerText.trim(),
      });
  ...
  return (
    <ScrollView contentContainerStyle={styles.scroll}>
      ...
      {list.length === 0 ? <Text style={styles.empty}>No doubts yet.</Text> :
        list.map(d => (
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
              ...
              <View style={[styles.statusPill,
                ...
                <Text style={[styles.statusPillTxt,
                  d.status === 'answered' ? { color: '#15803d' } : { color: '#92400e' }]}>{d.status}</Text>
              </View>
            </View>
      ...
      <Modal visible={!!answering} animationType="slide" transparent onRequestClose={() => setAnswering(null)}>
        <View style={styles.modalBg}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>Answer Doubt</Text>
            <Text style={styles.doubtQ}>{answering?.content}</Text>
            <TextInput style={styles.input} value={answerText} onChangeText={setAnswerText}
              placeholder="Your answer…" placeholderTextColor="#94a3b8" multiline />
            <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}>
              <TouchableOpacity style={[styles.btn, { backgroundColor: '#f1f5f9' }]} onPress={() => { setAnswering(null); setAnswerText(''); }}>
                <Text style={[styles.btnTxt, { color: '#475569' }]}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.btn, { backgroundColor: PRIMARY }]} onPress={submitAnswer}>
...
function PulseTab({ session_id, pulses, isLive }) {
  ...
  const create = async () => {
    ...
    try {
      const body = autoGen
        ? { trigger_type: 'manual', auto_generate: true, duration_seconds: parseInt(duration, 10) || 30 }
        : {
            trigger_type: 'manual',
            auto_generate: false,
            question_text: q.trim(),
            option_a: opts.A.trim(), option_b: opts.B.trim(),
            option_c: opts.C.trim(), option_d: opts.D.trim(),
      ...
      setShowCreate(false); setQ(''); setOpts({ A: '', B: '', C: '', D: '' });
    } catch (err) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.scroll}>
      ...
      {pulses.length === 0 ? <Text style={styles.empty}>No pulses yet.</Text> :
        pulses.map(p => (
          ...
          <View style={[styles.modalCard, { maxHeight: '90%' }]}>
            <ScrollView>
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                <TouchableOpacity style={[styles.toggle, autoGen && styles.toggleOn]} onPress={() => setAutoGen(true)}>
                  <Text style={[styles.toggleTxt, autoGen && styles.toggleTxtOn]}>🤖 AI Auto</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.toggle, !autoGen && styles.toggleOn]} onPress={() => setAutoGen(false)}>
              ...
              {!autoGen && (
                <>
                  ...
                  <TextInput style={styles.input} value={q} onChangeText={setQ} multiline placeholder="Question…" placeholderTextColor="#94a3b8" />
                  {['A', 'B', 'C', 'D'].map(k => (
                    <View key={k}>
                      <Text style={styles.label}>Option {k}</Text>
                      <TextInput style={styles.input} value={opts[k]} onChangeText={t => setOpts(p => ({ ...p, [k]: t }))} placeholder={`Option ${k}`} placeholderTextColor="#94a3b8" />
                    </View>
                  ))}
                  <Text style={styles.label}>Correct Answer</Text>
                  <View style={{ flexDirection: 'row', gap: 6 }}>
                    {['A', 'B', 'C', 'D'].map(k => (
                      <TouchableOpacity key={k} style={[styles.toggle, correct === k && styles.toggleOn]} onPress={() => setCorrect(k)}>
                        <Text style={[styles.toggleTxt, correct === k && styles.toggleTxtOn]}>{k}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </>
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 14 }}>
                <TouchableOpacity style={[styles.btn, { backgroundColor: '#f1f5f9' }]} onPress={() => setShowCreate(false)}>
                  <Text style={[styles.btnTxt, { color: '#475569' }]}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.btn, { backgroundColor: PRIMARY }, busy && { opacity: 0.6 }]} onPress={create} disabled={busy}>
                  {busy ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnTxt}>Launch</Text>}
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
...
function BrainTab({ session_id, isLive }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.scroll}>
      ...
      <Text style={styles.sub2}>Describe a concept; AI returns a structured explanation to project on the class wall.</Text>
      <TextInput style={[styles.input, { minHeight: 80 }]} value={prompt} onChangeText={setPrompt}
        placeholder="e.g. Explain TCP handshake with diagram steps" placeholderTextColor="#94a3b8" multiline editable={isLive} />
      <TouchableOpacity style={[styles.bigBtn, (!isLive || busy) && { opacity: 0.5 }]} onPress={generate} disabled={!isLive || busy}>
        {busy ? <ActivityIndicator color="#fff" /> :
          <><Ionicons name="sparkles-outline" size={16} color="#fff" /><Text style={styles.bigBtnTxt}>Generate & Broadcast</Text></>}
      </TouchableOpacity>
      {result && (
        <View style={styles.resultCard}>
          <Text style={styles.resultTitle}>Generated</Text>
          <Text selectable style={styles.resultBody}>
            {typeof result === 'string' ? result : (result.content ?? result.text ?? JSON.stringify(result, null, 2))}
          </Text>
        </View>
      )}
    </ScrollView>
    ...
    <ScrollView contentContainerStyle={styles.scroll}>
      ...
      {activeRooms.length === 0 ? (
        <>
          ...
          <TextInput style={styles.input} value={numRooms} onChangeText={setNumRooms} keyboardType="number-pad" />
          <TouchableOpacity style={[styles.bigBtn, (!isLive || busy) && { opacity: 0.5 }]} onPress={createRooms} disabled={!isLive || busy}>
            {busy ? <ActivityIndicator color="#fff" /> :
              <><Ionicons name="people-outline" size={16} color="#fff" /><Text style={styles.bigBtnTxt}>Create Rooms</Text></>}
          </TouchableOpacity>
        </>
      ) : (
        <>
          ...
          <TouchableOpacity style={[styles.bigBtn, { backgroundColor: '#ef4444' }]} onPress={endAll}>
...
const styles = StyleSheet.create({
  ...
  endBtn: { backgroundColor: '#ef4444', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8 },
  ...
  stat: { flex: 1, backgroundColor: '#fff', borderRadius: 10, padding: 12, alignItems: 'center', gap: 4, borderWidth: 1, borderColor: '#e2e8f0' },
  statV: { fontSize: 18, fontWeight: '800', color: '#1e293b' },
  statL: { fontSize: 10, color: '#94a3b8', fontWeight: '600' },
  actionBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#fff', padding: 10, borderRadius: 8, borderWidth: 1, borderColor: '#e2e8f0' },
  ...
  aiBox: { marginTop: 8, padding: 8, backgroundColor: '#eff6ff', borderRadius: 6, borderLeftWidth: 3, borderLeftColor: '#3b82f6' },
  tBox: { marginTop: 8, padding: 8, backgroundColor: '#f0fdf4', borderRadius: 6, borderLeftWidth: 3, borderLeftColor: '#22c55e' },
  aiLabel: { fontSize: 10, fontWeight: '800', color: '#475569', marginBottom: 2 },
  aiTxt: { fontSize: 12, color: '#1e293b', lineHeight: 17 },
  ansBtn: { flexDirection: 'row', alignSelf: 'flex-start', alignItems: 'center', backgroundColor: PRIMARY, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6, marginTop: 8, gap: 4 },
  ansBtnTxt: { color: '#fff', fontSize: 11, fontWeight: '700' },
  smallBtn: { flexDirection: 'row', backgroundColor: PRIMARY, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 6, alignItems: 'center', gap: 4 },
  ...
  distItem: { flex: 1, alignItems: 'center', backgroundColor: '#f8fafc', padding: 6, borderRadius: 6 },
  ...
  toggle: { flex: 1, padding: 8, borderRadius: 8, backgroundColor: '#f1f5f9', borderWidth: 1, borderColor: '#e2e8f0', alignItems: 'center' },
  ...
  bigBtn: { flexDirection: 'row', backgroundColor: PRIMARY, padding: 14, borderRadius: 10, alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 12 },
  bigBtnTxt: { color: '#fff', fontWeight: '700', fontSize: 14 },
  resultCard: { backgroundColor: '#fff', borderRadius: 12, padding: 14, marginTop: 14, borderWidth: 1, borderColor: '#e2e8f0' },
  resultTitle: { fontSize: 13, fontWeight: '700', color: PRIMARY, marginBottom: 8 },
  resultBody: { fontSize: 12, color: '#1e293b', lineHeight: 18 },
  roomCard: { backgroundColor: '#fff', borderRadius: 10, padding: 12, marginBottom: 6, borderWidth: 1, borderColor: '#e2e8f0' },
