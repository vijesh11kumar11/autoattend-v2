...
export default function AttendanceHistoryScreen({ navigation }) {
  ...
  const subjects = useMemo(() => {
    const set = new Map();
    items.forEach((s) => {
      if (s.subject_name) set.set(s.subject_code ?? s.subject_name, s.subject_name);
    });
  ...
  const filtered = useMemo(() => {
    return items.filter((s) => {
      ...
      if (search && !`${s.subject_name} ${s.date}`.toLowerCase().includes(search.toLowerCase()))
        return false;
      return true;
    });
  }, [items, subj, search]);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <Text style={styles.sub}>
          {items.length} session{items.length === 1 ? '' : 's'} conducted
        </Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 10 }}>
          {subjects.map(([k, label]) => (
            <TouchableOpacity
              key={k}
              style={[styles.chip, subj === k && styles.chipActive]}
              onPress={() => setSubj(k)}
            >
            ...
            return (
              <View key={s.id} style={styles.card}>
                <View style={styles.cardHead}>
                  ...
                  >
                    <Text style={[styles.badgeTxt, s.status === 'active' && { color: '#15803d' }]}>
                      {s.status ?? 'ended'}
                    </Text>
                  </View>
                </View>
