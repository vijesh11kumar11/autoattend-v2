...
class User(Base, SoftDeleteMixin):
    ...
    timetable_entries = relationship(
        "Timetable", back_populates="teacher", foreign_keys="[Timetable.teacher_id]"
    )
...
class Timetable(Base, SoftDeleteMixin, TenantMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "subject_id", "day_of_week", "start_time", name="uq_timetable_subject_day_start"
        ),
...
class AttendanceSession(Base, SoftDeleteMixin, TenantMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "subject_id", "date", "section_id", name="uq_attendance_session_subject_date_section"
        ),
    ...
    status = Column(
        SAEnum(SessionStatus, name="sessionstatus"), default=SessionStatus.active, nullable=False
    )
    ...
    linked_live_session_id = Column(
        Integer, ForeignKey("live_sessions.id", ondelete="SET NULL"), nullable=True
    )
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    ...
    face_verify_tokens = relationship(
        "FaceVerifyToken", back_populates="session", cascade="all, delete-orphan"
    )
    attendance_records = relationship(
        "AttendanceRecord", back_populates="session", cascade="all, delete-orphan"
    )
    audit_logs = relationship(
        "AttendanceAudit", back_populates="session", cascade="all, delete-orphan"
    )
...
class TWMSession(Base, SoftDeleteMixin, TenantMixin):
    ...
    status = Column(
        SAEnum(SessionStatus, name="sessionstatus", create_type=False), default=SessionStatus.active
    )
    ...
    attendance = relationship(
        "TWMAttendance", back_populates="session", cascade="all, delete-orphan"
    )
...
class LeaveRequest(Base, SoftDeleteMixin, TenantMixin):
    ...
    status = Column(
        SAEnum(LeaveRequestStatus, name="leaverequeststatus"), default=LeaveRequestStatus.pending
    )
...
class AttendanceDispute(Base, SoftDeleteMixin, TenantMixin):
    ...
    status = Column(
        SAEnum(DisputeStatus, name="disputestatus"), default=DisputeStatus.pending, nullable=False
    )
...
class CareerRoadmap(Base, SoftDeleteMixin, TenantMixin):
    ...
    __table_args__ = (Index("ix_career_roadmaps_user_id", "user_id"),)
...
class SuggestionAIReport(Base, SoftDeleteMixin, TenantMixin):
    ...
    __table_args__ = (Index("ix_suggestion_reports_scope", "scope", "scope_id"),)
...
class Capsule(Base, SoftDeleteMixin, TenantMixin):
    ...
    capsule_type = Column(
        SAEnum(CapsuleType, name="capsuletype"), default=CapsuleType.notes, nullable=False
    )
    ...
    unlock_mode = Column(
        SAEnum(CapsuleUnlockMode, name="capsuleunlockmode"),
        default=CapsuleUnlockMode.always,
        nullable=False,
    )
    ...
    source_live_session_id = Column(
        Integer,
        ForeignKey(
            "live_sessions.id",
            ondelete="SET NULL",
            use_alter=True,
            name="fk_capsules_source_live_session_id",
        ),
        nullable=True,
    )
    ...
    interactions = relationship(
        "CapsuleInteraction", back_populates="capsule", cascade="all, delete-orphan"
    )
    wall_posts = relationship("ClassWallPost", back_populates="capsule")
    source_live_session = relationship(
        "LiveSession", foreign_keys=[source_live_session_id], post_update=True
    )
    access_logs = relationship(
        "CapsuleAccessLog", back_populates="capsule", cascade="all, delete-orphan"
    )
...
class ClassWallPost(Base, SoftDeleteMixin, TenantMixin):
    ...
    status = Column(
        SAEnum(WallPostStatus, name="wallpoststatus"), default=WallPostStatus.open, nullable=False
    )
    ...
    resonances = relationship(
        "ClassWallResonance", back_populates="post", cascade="all, delete-orphan"
    )
...
class LiveSession(Base, SoftDeleteMixin, TenantMixin):
    ...
    status = Column(
        SAEnum(LiveSessionStatus, name="livesessionstatus"),
    ...
    events = relationship(
        "LiveSessionEvent", back_populates="live_session", cascade="all, delete-orphan"
    )
    pulse_checks = relationship(
        "PulseCheck", back_populates="live_session", cascade="all, delete-orphan"
    )
    breakout_rooms = relationship(
        "LiveSessionBreakoutRoom", back_populates="live_session", cascade="all, delete-orphan"
    )
...
class LiveSessionParticipant(Base, SoftDeleteMixin, TenantMixin):
    ...
    participant_type = Column(
        SAEnum(LiveParticipantType, name="liveparticipanttype"), nullable=False
    )
    ...
    connection_quality = Column(
        SAEnum(LiveConnectionQuality, name="liveconnectionquality"),
        default=LiveConnectionQuality.good,
        nullable=False,
    )
...
class LivePulseCheck(Base, SoftDeleteMixin, TenantMixin):
    ...
    __table_args__ = (Index("ix_live_pulse_checks_session_id", "live_session_id"),)
...
class LivePulseResponse(Base, SoftDeleteMixin, TenantMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    pulse_id = Column(
        Integer, ForeignKey("live_pulse_checks.id", ondelete="CASCADE"), nullable=False
    )
    live_session_id = Column(
        Integer, ForeignKey("live_sessions.id", ondelete="CASCADE"), nullable=False
    )
    participant_id = Column(
        Integer, ForeignKey("live_session_participants.id", ondelete="SET NULL"), nullable=True
    )
...
class StudentKnowledgeGraph(Base, SoftDeleteMixin, TenantMixin):
    ...
    __table_args__ = (
        UniqueConstraint(
            "student_id", "subject_id", "topic_name", name="uq_skg_student_subject_topic"
        ),
    ...
    understanding_level = Column(
        SAEnum(KnowledgeLevel, name="knowledgelevel"),
        default=KnowledgeLevel.not_covered,
        nullable=False,
    )
    confidence_score = Column(Float, default=0.0, nullable=False)
    last_assessed_session_id = Column(
        Integer, ForeignKey("live_sessions.id", ondelete="SET NULL"), nullable=True
    )
...
class LiveSessionBreakoutRoom(Base, SoftDeleteMixin, TenantMixin):
    ...
    __table_args__ = (Index("ix_lsbr_live_session_id", "live_session_id"),)
    ...
    peer_expert_detected_user_id = Column(
        Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True
    )
