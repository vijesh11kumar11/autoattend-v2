...
TITLE_STYLE = ParagraphStyle(
    ...
    fontSize=16, textColor=C_HEADER, spaceAfter=4,
)
SUBTITLE_STYLE = ParagraphStyle(
    "subtitle", parent=STYLES["Normal"],
    fontSize=9, textColor=C_GREY, spaceAfter=12,
)
...
def _status_color(pct: float, threshold: float) -> colors.Color:
    if pct >= threshold:          return C_GREEN
    if pct >= threshold - 10:     return C_AMBER
    return C_RED
...
def generate_student_pdf(
    ...
    from database import (
        ...
        Course, SessionStatus, Subject, User, UserRole,
    )
    ...
    session_q = (
        ...
        .filter(AttendanceSession.status == SessionStatus.ended)
    )
    if date_from:
    ...
    _header_block(story, "Student Attendance Report", [
    ...
    for r in rows:
        ...
        total_p += r.present;  total_a += r.absent;  total_t += r.total
        st = "SAFE" if pct >= settings.ATTENDANCE_THRESHOLD else (
             "WARNING" if pct >= settings.ATTENDANCE_THRESHOLD - 10 else (
             "CRITICAL" if pct >= settings.ATTENDANCE_THRESHOLD - 25 else "DETAINED"))
        data.append([r.subject_name, r.code, r.present, r.absent, r.total,
                     f"{pct}%", st])

    overall_pct = round(total_p * 100 / total_t, 1) if total_t else 0.0
    data.append(["TOTAL", "", total_p, total_a, total_t,
                 f"{overall_pct}%", ""])
...
def generate_class_session_pdf(session_id: int, db: Session) -> io.BytesIO:
    ...
    _header_block(story, "Class Session Attendance Report", [
...
def generate_defaulters_pdf(
    ...
    if not student_ids or not subject_ids:
        ...
        story = []
        _header_block(story, "Defaulters Report", [
    ...
    if not rows:
        story.append(Paragraph(
            f"No students below {threshold}% attendance threshold.", BODY_STYLE,
        ))
    else:
        data = [["#", "Roll No", "Name", "Subject", "Present", "Total", "Attendance %", "Status"]]
        for idx, r in enumerate(rows, 1):
            pct = round(r.present * 100 / r.total, 1) if r.total else 0.0
            st  = ("DETAINED" if pct < threshold - 25
                   else "CRITICAL" if pct < threshold - 10
                   else "WARNING")
            data.append([idx, r.roll_number or "—", r.student_name,
                         r.subject_name, r.present, r.total, f"{pct}%", st])
...
def generate_monthly_excel(
    ...
    from database import (
        AttendanceRecord, AttendanceSession, AttendanceStatus,
        SessionStatus, Subject, User, UserRole,
    )
    from config import settings
    ...
    for col_idx, sess in enumerate(sessions, start=4):
        cell = ws.cell(HDR_ROW, col_idx, f"{sess.date.strftime('%d/%m')}\n{str(sess.start_time)[:5]}")
    ...
    # Data rows
    status_fill = {"P": FILL_GREEN, "A": FILL_RED, "L": FILL_AMBER, "M": FILL_BLUE, "D": FILL_PURPLE}
    ...
    for row_idx, stu in enumerate(students, start=HDR_ROW + 1):
        ...
        pct_cell.font      = Font(bold=True, size=8,
                                   color="166534" if pct >= settings.ATTENDANCE_THRESHOLD
                                         else ("92400E" if pct >= settings.ATTENDANCE_THRESHOLD - 10
                                               else "991B1B"))
    ...
    items = [("P", "Present", FILL_GREEN), ("A", "Absent", FILL_RED),
             ("L", "Late", FILL_AMBER), ("M", "Medical Leave", FILL_BLUE),
             ("D", "Duty Leave", FILL_PURPLE)]
    for i, (code, label, fill) in enumerate(items):
        c = ws.cell(legend_row, 2 + i * 2, code)
        c.fill = fill; c.font = Font(bold=True, size=8); c.alignment = ALIGN_CENTER
