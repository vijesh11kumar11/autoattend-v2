...
def _create_guest_token(session_id: int, participant_id: int, guest_name: str) -> str:
    expire = datetime.now(UTC) + timedelta(hours=GUEST_TOKEN_TTL_HOURS)
    payload = {
...
def _section_student_ids(db: Session, section_id: Optional[int]) -> list[int]:
    if section_id is None:
        return []
    rows = (
        db.query(User.id).filter(User.role == UserRole.student, User.section_id == section_id).all()
    )
...
async def _generate_session_capsule_async(session_id: int) -> None:
    from database import CapsuleInteraction, SessionLocal
    ...
    try:
        ...
        events_payload = [
            {
                ...
                "triggered_by": (
                    e.triggered_by.value
                    if hasattr(e.triggered_by, "value")
                    else str(e.triggered_by)
                ),
            }
        ...
        session_data = {
            ...
            "session_type": (
                ...
                if hasattr(sess.session_type, "value")
                else str(sess.session_type)
            ),
        }
        ...
        attending_ids = [
            p.user_id
            for p in db.query(LiveSessionParticipant)
            .filter(
                LiveSessionParticipant.live_session_id == session_id,
...
async def _generate_health_report_async(session_id: int) -> None:
    ...
    try:
        ...
        pulses = db.query(PulseCheck).filter(PulseCheck.live_session_id == session_id).all()
        events = (
            db.query(LiveSessionEvent).filter(LiveSessionEvent.live_session_id == session_id).all()
        )
        doubts = db.query(ClassWallPost).filter(ClassWallPost.live_session_id == session_id).count()
        session_data = {
            "title": sess.title,
            "duration_minutes": sess.duration_minutes,
            "participants": [
                {
                    ...
                    "type": (
                        ...
                        if hasattr(p.participant_type, "value")
                        else str(p.participant_type)
                    ),
...
def _update_all_knowledge_graphs_sync(session_id: int) -> None:
    ...
    try:
                ...
                pulse_results.append(
                    {
                        ...
                        "is_correct": (p.correct_responses or 0)
                        >= max(1, (p.total_responses or 0) // 2),
                    }
                )
            try:
                update_student_knowledge_graph(
        ...
        logger.info(
            "Knowledge graphs updated for session %s (%d students)", session_id, len(student_ids)
        )
    except Exception as exc:
...
def _notify_absent_students_async(session_id: int) -> None:
    ...
    try:
        ...
        subject_name = (
            db.query(Subject.name).filter(Subject.id == sess.subject_id).scalar() or "Subject"
        )
...
def create_session(
    ...
    if body.session_type == "capsule_locked":
        if not body.capsule_id:
            raise HTTPException(
                ...
                "capsule_id required for capsule_locked sessions.",
            )
    ...
    if body.session_type == "standalone":
        if not subject_id or not section_id:
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY,
                "subject_id and section_id required for standalone sessions.",
            )
...
def start_live_session(
    ...
    _log_event(
        db,
        sess.id,
        LiveEventType.session_start,
        LiveEventTrigger.teacher,
...
def join_live_session(
    ...
    if user is None:
        if not sess.allow_guests:
            raise HTTPException(
                status.HTTP_403_FORBIDDEN, "This is a private session. Please login."
            )
        ...
        _log_event(
            db,
            sess.id,
            LiveEventType.student_joined,
            LiveEventTrigger.system,
    ...
    if sess.session_type == LiveSessionType.standalone:
        ...
        capsule = (
            db.query(Capsule).filter(Capsule.id == sess.capsule_id).first()
            if sess.capsule_id
            else None
        )
        ...
        if role == "student" and not _is_student_in_section(db, user_id, target_section):
            ...
            section = (
                db.query(Section).filter(Section.id == target_section).first()
                if target_section
                else None
            )
            raise HTTPException(
    ...
    p_type = (
        LiveParticipantType.teacher
        if role == "teacher" and sess.teacher_id == user_id
        else (LiveParticipantType.student if role == "student" else LiveParticipantType.external)
    )
...
def heartbeat(
    ...
    if part.user_id and part.participant_type == LiveParticipantType.student and session_active:
        ...
        if (
            not last_check
            or (now - last_check).total_seconds() > settings.LIVE_SESSION_LIVENESS_CHECK_INTERVAL
        ):
            needs_liveness = True
...
async def ai_observation(
    ...
    context = {
        ...
        "previous_observations": [
            r.ai_observation_text for r in prev_obs_rows if r.ai_observation_text
        ],
    }
...
async def ai_whiteboard(
    ...
    schema = '{"diagram_type": "mermaid|html", ' '"diagram_code": "string", ' '"title": "string"}'
    prompt = (
    ...
    result = await _ai_json(
        prompt, system="You are an instructor designing whiteboard visuals. Return only valid JSON."
    )
    ...
    _log_event(
        db,
        session_id,
        LiveEventType.whiteboard_generated,
        LiveEventTrigger.teacher,
...
def get_participant_names(
    ...
    if not authorised and credentials and credentials.credentials:
        ...
        try:
            ...
            if (
                payload.get("purpose") == "live_guest"
                and int(payload.get("session_id") or 0) == session_id
            ):
                authorised = True
        except Exception:
...
async def pulse_create(
    ...
    if body.auto_generate or trigger_type == "ai":
        ...
        if not gen.get("question"):
            raise HTTPException(
                status.HTTP_502_BAD_GATEWAY, "AI failed to generate a pulse-check question."
            )
        qdata = gen
        ...
        if not all(
            ...
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY,
                "All question fields required for manual pulse check.",
            )
        if body.correct_answer.upper() not in ("A", "B", "C", "D"):
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY, "correct_answer must be A|B|C|D."
            )
        qdata = {
    ...
    _log_event(
        db,
        session_id,
        LiveEventType.pulse_check_started,
        (
            LiveEventTrigger.teacher
            if triggered_by_enum == PulseCheckTrigger.teacher
            else LiveEventTrigger.ai
        ),
...
def pulse_respond(
    ...
    elapsed = (datetime.now(UTC) - pulse.triggered_at).total_seconds()
    if elapsed > pulse.duration_seconds:
        pulse.closed_at = datetime.now(UTC)
        db.commit()
...
def pulse_results(
    ...
    closed = pulse.closed_at is not None or (
        (datetime.now(UTC) - pulse.triggered_at).total_seconds() > pulse.duration_seconds
    )
...
async def post_doubt(
    ...
    if not sess.subject_id:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "Session has no subject; cannot post doubt."
        )
    ...
    capsule = (
        db.query(Capsule).filter(Capsule.id == sess.capsule_id).first() if sess.capsule_id else None
    )
    ai_suggestion = None
    confidence = 0.0
    try:
...
async def pre_class_brief(
    ...
    if not sess.subject_id:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "Session has no subject; pre-class brief unavailable."
        )
    context = {
        "title": sess.title,
        "session_type": sess.session_type.value,
...
async def health_report(
    ...
    doubts_score = (
        min(100, doubts_resolved / max(1, doubts_posted) * 100) if doubts_posted else 80.0
    )
    overall = round(
    ...
    return {
        ...
        "metrics": {
            "attendance": {
                ...
                "label": f"{attended}/{total_participants} students",
            },
...
def student_report(
    ...
    return {
        ...
        "attendance_counted": counted,
        "doubts_posted": [
            {"id": d.id, "content": d.content, "status": d.status.value} for d in doubts
        ],
...
async def breakout_create(
    ...
    _log_event(
        db,
        session_id,
        LiveEventType.breakout_started,
        LiveEventTrigger.teacher,
...
def breakout_status_aggregate(
    ...
    cutoff = datetime.now(UTC) - timedelta(minutes=4)
    out: list[dict] = []
    ...
    for r in rooms:
        ...
        parts = db.query(LiveSessionParticipant).filter(LiveSessionParticipant.id.in_(pids)).all()
        user_ids = [p.user_id for p in parts if p.user_id]
...
def _kg_response(
    rows: list[StudentKnowledgeGraph], student_name: str, subject_name: Optional[str]
) -> dict:
    if not rows:
        return {
...
def my_knowledge_graph(
    ...
    for sid, srows in by_subject.items():
        ...
        payload = _kg_response(
            srows, current_user.get("name", "Student"), subj.name if subj else None
        )
...
def analytics_teacher_summary(
    ...
    avg_attendance = (
        round(sum(attendance_avgs) / len(attendance_avgs), 1) if attendance_avgs else 0.0
    )
...
def analytics_hod_overview(
    ...
    return {
        ...
        "sessions_per_subject": [{"subject_id": sid, "count": c} for sid, c in per_subject.items()],
...
def _pulse_counts(pulse_id: int, db: Session) -> dict:
    rows = db.query(LivePulseResponse).filter(LivePulseResponse.pulse_id == pulse_id).all()
...
async def _auto_close_live_pulse(pulse_id: int, delay_secs: int) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                "pulse_check_closed broadcast failed for pulse %s", pulse_id, exc_info=True
            )
    except Exception as exc:
...
async def close_live_pulse_check(
    ...
    try:
        ...
        logger.debug(
            "pulse_check_closed broadcast failed for session %s", session_id, exc_info=True
        )
    return final
...
def get_live_pulse_results(
    ...
    pulses = (
        ...
        .order_by(LivePulseCheck.sent_at.asc())
        .all()
    )
...
async def _generate_observation_payload(session_id: int, db: Session) -> Optional[dict]:
    ...
    elapsed_mins = 0
    if sess.started_at:
        elapsed_mins = max(0, int((datetime.now(UTC) - sess.started_at).total_seconds() / 60))
    ...
    if pulses:
        comps = [
            round(p.correct_count / p.total_responses * 100, 1) for p in pulses if p.total_responses
        ]
        if comps:
            pulse_avg = round(sum(comps) / len(comps), 1)

    engagement_pct = (
        round((active_participants / total_participants) * 100, 0) if total_participants else 0
    )
...
{schema}"""
    ...
    try:
        result = await _ai_json(
            prompt, system="You are an empathetic AI teaching assistant. Return only valid JSON."
        )
    except Exception as exc:
    ...
    try:
        ...
        if payload.get("type") in bookmark_map:
            ...
            if sess_row and sess_row.started_at:
                elapsed_secs = int((datetime.now(UTC) - sess_row.started_at).total_seconds())
...
async def list_ai_observations(
    ...
    if sess.status == LiveSessionStatus.live:
        try:
            ...
            logger.warning(
                "Failed to ensure observation scheduler for session %s", session_id, exc_info=True
            )
...
Max 60 words. Plain text, no markdown."""
    ...
    elapsed_mins = 0
    if sess.started_at:
        elapsed_mins = max(0, int((datetime.now(tz=UTC) - sess.started_at).total_seconds() / 60))
...
def get_student_attention(
    ...
    engagements = (
        db.query(LiveStudentEngagement).filter(LiveStudentEngagement.session_id == session_id).all()
    )
    ...
    if part_ids:
        for p in (
            db.query(LiveSessionParticipant).filter(LiveSessionParticipant.id.in_(part_ids)).all()
        ):
            parts_map[p.id] = p
...
async def check_ai_intervention(
    ...
    if pulses:
        comps = [
            (p.correct_count or 0) / p.total_responses * 100.0 for p in pulses if p.total_responses
        ]
        if comps:
    ...
    session_data = {
        "elapsed_mins": elapsed_mins,
        "engagement_pct": (
            round(((total_students - silent_count) / total_students * 100.0), 1)
            if total_students
            else 100.0
        ),
...
class AddBookmarkRequest(BaseModel):
    bookmark_type: str = Field(
        ..., pattern=r"^(topic_start|confusion|clarity|live_demo|qa_start|break|other)$"
    )
...
async def add_bookmark(
    ...
    elapsed_mins = 0
    if sess.started_at:
        elapsed_secs = int((datetime.now(UTC) - sess.started_at).total_seconds())
        elapsed_mins = elapsed_secs // 60
