...
def detect_anomalies(
    ...
    for sess_id, sess_recs in recs_by_session.items():
        gps_recs = [r for r in sess_recs if r.student_latitude and r.student_longitude and r.marked_at]
    ...
    for sid in student_ids:
        ...
        first_present = sum(1 for r in first_half if r.status in (AttendanceStatus.present, AttendanceStatus.late))
        first_pct = first_present / len(first_half) * 100 if first_half else 0
        second_present = sum(1 for r in second_half if r.status in (AttendanceStatus.present, AttendanceStatus.late))
    ...
    return {
        "anomalies": sorted(anomalies, key=lambda a: {"high": 0, "medium": 1, "low": 2}.get(a["severity"], 3)),
...
def attendance_forecast(
    ...
    for subj_id, recs in by_subject.items():
        ...
        attended = sum(1 for r in recs if r.status in (AttendanceStatus.present, AttendanceStatus.late))
        total = len(recs)
