...
def start_session(
    ...
    """
    subject: Optional[Subject] = db.query(Subject).filter(Subject.id == body.subject_id).first()
    if not subject:
    ...
    logger.info(
        "📗 SESSION START │ teacher_id=%d │ subject='%s' (id=%d) │ date=%s",
        caller_id,
        subject.name,
        body.subject_id,
        body.date,
    ...
    logger.info(
        "📗 SESSION START │ teacher GPS lat=%.6f lon=%.6f",
        body.teacher_latitude or 0.0,
        body.teacher_longitude or 0.0,
    )
    logger.info(
        "📗 SESSION START │ QR secret generated (hint=%s...) │ BLE token generated (%d chars)",
        qr_secret[:8],
        len(bt_token),
    )
    ...
    logger.info(
        "📗 SESSION START │ session_id=%d │ %d students enrolled (default=absent)",
        session.id,
        len(students),
    )
...
def mark_attendance(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(
        "📝 MARK ATTENDANCE │ student_id=%d │ session_id=%d │ IP=%s",
        student_id,
        body.session_id,
        ip,
    )
    ...
    if getattr(body, "is_rooted", False):
        sec_logger.log(
            SecurityEventType.ROOTED_DEVICE_DETECTED,
            Severity.CRITICAL,
            user_id=student_id,
            ip_address=ip,
    ...
    if session.status != SessionStatus.active:
        ...
        logger.warning(
            "❌ STEP 1a │ session_id=%d │ already closed (status=%s)",
            body.session_id,
    ...
    logger.info(
        "✅ STEP 1a │ Session found │ id=%d │ status=%s │ subject_id=%d",
        session.id,
        session.status.value,
        session.subject_id,
    )
    ...
    if subject is None:
        ...
        logger.error(
            "❌ STEP 1a │ session_id=%d │ subject_id=%d missing — refusing to mark",
            session.id,
            session.subject_id,
        )
        raise HTTPException(
    ...
    if (
        ...
        logger.warning(
            "❌ STEP 1b │ student_id=%d │ not enrolled in subject_id=%d",
            student_id,
            session.subject_id,
        )
        raise HTTPException(
    ...
    logger.info(
        "✅ STEP 1b │ Student enrolled │ roll=%s │ subject='%s'", student.roll_number, subject.name
    )
    ...
    if record and record.status in _finalized:
        ...
        logger.info(
            "ℹ️ STEP 1c │ student_id=%d │ already finalised as %s — skipping",
            student_id,
            record.status.value,
        )
    ...
    logger.info(
        "📱 STEP 2 │ Device check │ JWT_device=%s │ body_device=%s",
        current_user.get("device_id", "none"),
        body.device_id or "none",
    )
    if current_user.get("device_id") and body.device_id:
        if current_user["device_id"] != body.device_id:
            checks["device_matched"] = False
            logger.warning(
                "❌ STEP 2 │ DEVICE MISMATCH │ student_id=%d │ expected=%s │ got=%s",
                student_id,
    ...
    logger.info("🙍 STEP 3 │ Face token validation │ token=%s...", (body.face_token or "")[:16])
    face_valid = validate_face_verify_token(body.face_token, student_id, body.session_id, db)
    if not face_valid:
    ...
    if not qr_result.get("valid"):
        ...
        _write_audit(
            ...
            student_id,
            AuditResult.failed,
            qr_result.get("reason", "QR invalid"),
            0.0,
            None,
            body.device_id,
            ip,
    ...
    if _is_test_roll:
        ...
        logger.info(
            "📍 STEP 5 │ Student  GPS │ lat=%.6f │ lon=%.6f │ accuracy=%.1f m",
            body.student_latitude or 0.0,
            body.student_longitude or 0.0,
        ...
        gps_result = verify_gps_proximity(
            ...
            previous_lat=(
                prev_snap.latitude
                if (
                    prev_snap := db.query(StudentGPSSnapshot)
                    .filter(StudentGPSSnapshot.user_id == student_id)
                    .first()
                )
                else None
            ),
        ...
        if not gps_result.get("verified"):
            logger.warning(
                "❌ STEP 5 │ GPS FAILED │ distance=%s m │ reason=%s",
                gps_result.get("distance_meters", "N/A"),
                gps_result.get("reason", "unknown"),
            )
            _write_audit(
                db,
                session.id,
                ...
                0.0,
                gps_result.get("distance_meters"),
                body.device_id,
                ip,
            )
            reason = gps_result.get("reason", "")
            is_spoof = (
                "mock" in reason.lower()
                or "velocity" in reason.lower()
                or "accuracy" in reason.lower()
            )
            sec_logger.log(
                (
                    SecurityEventType.GPS_SPOOFING_DETECTED
                    if is_spoof
                    else SecurityEventType.ATTENDANCE_FRAUD_SUSPECTED
                ),
        ...
        logger.info(
            "✅ STEP 5 │ GPS VERIFIED │ distance=%.1f m │ accuracy=%.1f m │ suspicious=%s",
            gps_distance or 0.0,
            body.student_gps_accuracy or 0.0,
            gps_flagged,
        )
    ...
    logger.info(
        "📶 STEP 6 │ Bluetooth check │ seed=%s... │ detected_token=%s...",
        bt_secret[:8],
        (body.bluetooth_token_detected or "")[:8],
    )
    ...
    if not bt_verified:
        if settings.BLUETOOTH_REQUIRED:
            logger.warning(
                "❌ STEP 6 │ BLUETOOTH FAILED │ reason=%s", bt_result.get("reason", "unknown")
            )
            _write_audit(
                db,
                session.id,
            ...
            sec_logger.log(
                SecurityEventType.BLE_MISMATCH,
                Severity.WARN,
                user_id=student_id,
                ip_address=ip,
...
def end_session(
    ...
    # Count results
    all_records = db.query(AttendanceRecord).filter(AttendanceRecord.session_id == session_id).all()
    ...
    try:
        ...
        if sa_count:
            present_ids = [
                r.student_id for r in all_records if r.status == AttendanceStatus.present
            ]
...
def get_session_status(
    ...
    students_map = {u.id: u for u in db.query(User).filter(User.id.in_(student_ids)).all()}
...
def manual_override(
    ...
    if session.status == SessionStatus.ended and session.end_time:
        ...
        ended_dt = datetime.combine(session.date, session.end_time).replace(tzinfo=UTC)
        if datetime.now(tz=UTC) > ended_dt + timedelta(hours=_OVERRIDE_WINDOW_HOURS):
            raise HTTPException(
...
def auto_expire_sessions(db: Session) -> int:
    ...
    cutoff = datetime.now(tz=UTC) - timedelta(minutes=CLASS_DURATION_MINUTES)
    ...
    for sess in stale_sessions:
        all_records = (
            db.query(AttendanceRecord).filter(AttendanceRecord.session_id == sess.id).all()
        )
        present = sum(1 for r in all_records if r.status == AttendanceStatus.present)
        total = len(all_records)
